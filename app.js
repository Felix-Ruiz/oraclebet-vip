        ...
        <div class="p-4 bg-gray-900 min-h-screen pb-20">
            ...
            <div id="vistaAdm_ladder" class="admin-view-content hidden space-y-4">
                ...
                <div class="bg-black/60 p-4 rounded-2xl border border-white/5 shadow-md mt-4">
                    <div class="flex justify-between items-center mb-3"><h3 class="text-[10px] text-gray-400 font-bold uppercase"><i class="fas fa-history text-blue-500 mr-1"></i> Historial de Retos</h3><button onclick="window.cargarHistorialEscaleraAdmin()" class="text-gray-500 hover:text-white p-1"><i class="fas fa-sync-alt"></i></button></div>
...
// 🚀 NUEVA FUNCIÓN: EL USUARIO EDITA SU CAPITAL DE ESCALERA LOCALMENTE
window.editarMiCapitalEscalera = function() {
    ...
    if(!isNaN(val) && val >= 1000) {
        ...
        window.cargarRetoEscaleraNube();
        window.mostrarAlerta("Guardado", "Tu fondo inicial ha sido actualizado.", "success");
    } else if (n !== null) {
            ...
            if(tk && tk.picks) { 
                ...
                <div class="bg-black/50 p-4 rounded-xl border border-yellow-500/50 shadow-[0_0_15px_rgba(212,175,55,0.2)] mb-4 relative overflow-hidden">
                    ...
                    <div class="grid grid-cols-3 gap-2 mb-4">
                        <div class="bg-gray-900 border border-white/5 p-2 rounded-xl text-center flex flex-col justify-center shadow-inner relative group cursor-pointer" onclick="window.editarMiCapitalEscalera()">
                            <div class="absolute inset-0 bg-black/40 hidden group-hover:flex items-center justify-center rounded-xl"><i class="fas fa-pencil-alt text-yellow-500 text-lg"></i></div>
                            <span class="text-[7px] text-gray-500 uppercase font-bold tracking-wider mb-1">Mi Capital</span>
...
window.iniciarApadrinamiento = async function() {
    ...
    try {
        await setDoc(doc(db, "codigos_nube", usuarioActual), { apadrinamiento: { bankroll_inicial: monto, bankroll_actual: monto, fecha_inicio: Date.now() } }, { merge: true });
...
window.generarRetoAdmin = async function() {
    ...
    try {
        ...
        window.retoPendientePublicar = { picks: seleccionados.map(s => ({ home_team: s.partido.home_team, away_team: s.partido.away_team, mercadoKey: s.mercadoKey, nombre: s.nombre, point: s.point === undefined ? null : s.point, cuota: s.cuota, probabilidad: s.probabilidad, stake: 100 })), cuotaTotal: cuotaAcum.toFixed(2), fechaFiltro: fechaElegida };
        ...
        previewHtml += `</div>`; document.getElementById('previewRetoAdmin').innerHTML = previewHtml; document.getElementById('previewRetoAdmin').classList.remove('hidden'); document.getElementById('inputAdminReto').value = `⚠️ Gestión de Banca Sugerida: Respeta el % indicado en cada pick para no perder todo en un solo fallo.`;
...
window.publicarRetoEscalera = async function() {
    ...
    window.retoPendientePublicar.picks = window.retoPendientePublicar.picks.map(p => ({ ...p, estado: 'pendiente', stake: 100 })); // Stake por defecto 100%
...
// 🚀 NUEVA FUNCIÓN: EL ADMIN EDITA EL % APOSTADO POR PICK
    ...
    if(!isNaN(val) && val > 0 && val <= 100) {
        try {
            ...
            if(snap.exists()) {
                ...
                window.cargarGestionRetoActivoAdmin();
                window.mostrarAlerta("Éxito", "Porcentaje de inversión ajustado.", "success");
            }
...
// 🚀 PANEL DE CONTROL EN VIVO PARA EL ADMIN
        ...
        if(snap.exists()) {
            ...
            tk.picks.forEach((p, index) => {
                ...
                if (p.estado === 'won') {
                    ...
                    currentCap = loseCap;
                    endPctGlobal = (loseCap / capitalInicial) * 100; // Si se pierde, la proyección termina en lo que quedó
                }
            ...
            html += `<div class="grid grid-cols-3 gap-2 mb-4">
                <div class="bg-gray-900 border border-white/5 p-2 rounded-lg text-center flex flex-col justify-center"><span class="block text-[7px] text-gray-500 uppercase tracking-wider">C. Inicial</span><span class="text-gray-400 font-black text-xs">${formatoCOP(capitalInicial)}</span></div>
                <div class="bg-black border ${tk.estado_reto === 'perdido' ? 'border-red-500/30' : 'border-yellow-500/50'} p-2 rounded-lg text-center flex flex-col justify-center"><span class="block text-[7px] ${tk.estado_reto === 'perdido' ? 'text-red-500' : 'text-yellow-500'} uppercase tracking-wider">Fondo Actual</span><span class="${tk.estado_reto === 'perdido' ? 'text-red-500' : 'text-yellow-500'} font-black text-sm">${formatoCOP(currentCap)}</span></div>
                <div class="bg-blue-900/20 border border-blue-500/30 p-2 rounded-lg text-center flex flex-col justify-center"><span class="block text-[7px] text-blue-400 uppercase tracking-wider">Crecimiento Global</span><span class="text-blue-500 font-black text-sm">${totalPctReal.toFixed(1)}%</span></div>
            </div>`;
...
window.crearCodigo = async function(esIlimitado) {
    ...
    try {
        ...
        if (docSnap.exists()) { return window.mostrarAlerta("Error", "Este código ya existe en la base de datos.", "error"); }
        const ahora = Date.now();
        await setDoc(ref, { code: nuevoCodigo, ilimitado: esIlimitado, deviceID: null, ladderStatus: 'none', creado: ahora });
        inputCode.value = ''; window.mostrarAlerta("Éxito", `Código ${esIlimitado ? 'PREMIUM' : 'VIP'} creado.`, "success");
        if(window.renderizarListaAdmin) window.renderizarListaAdmin();
    } catch (error) { console.error("Error creando código:", error); window.mostrarAlerta("Error", "Fallo de conexión al servidor.", "error"); }
};
...
window.renderizarUsuariosFiltrados = function(filtro = '') {
    ...
    let html = '';
    filtrados.forEach(u => {
        html += `<button onclick="window.verDetalleUsuarioAdmin('${u.uid}')" class="w-full bg-black/40 hover:bg-black/60 border border-white/5 p-4 rounded-xl flex justify-between items-center transition-all shadow-sm active:scale-95 mb-2"><div class="flex items-center gap-3"><div class="bg-gray-800 p-3 rounded-full text-yellow-500 shadow-inner"><i class="fas fa-user-astronaut text-lg"></i></div><div class="flex flex-col items-start"><span class="text-white font-black text-xs uppercase tracking-wider">${u.uid}</span><span class="text-gray-400 text-[9px] font-bold mt-0.5 px-2 bg-gray-800 rounded">${u.count} Tickets Guardados</span></div></div><i class="fas fa-chevron-right text-gray-600"></i></button>`;
    });
