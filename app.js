...
const firebaseConfig = { apiKey: "AIzaSyAUB1hC-JSQcrJ68ayl9rEUhP4Qz9xz7Uk", authDomain: "oraclebet-db.firebaseapp.com", projectId: "oraclebet-db", storageBucket: "oraclebet-db.firebasestorage.app", messagingSenderId: "407519995167", appId: "1:407519995167:web:4428c165b527ebb9c6fc2a" };
const app = initializeApp(firebaseConfig); const db = getFirestore(app); const auth = getAuth(app); const messaging = getMessaging(app);

window.procesarEnlaceInterno = function(urlStr) { if(!urlStr) return; if(urlStr.includes('view=escalera')) { window.cerrarBandejaNotificaciones(); window.cambiarVista('escalera'); if(!modoVipActivo) { window.mostrarAlerta("Acceso VIP", "Inicia sesión para ver el Reto Oficial de Escalera.", "info"); window.abrirModalLogin(); } else { window.chequearEstadoEscaleraUI(); } } else if (urlStr.includes('inbox=true')) { window.abrirBandejaNotificaciones(); } };
if ('serviceWorker' in navigator) { navigator.serviceWorker.register('./firebase-messaging-sw.js').then((registration) => { console.log('✅ SW registrado.'); }).catch((err) => { console.error('❌ Error SW:', err); }); navigator.serviceWorker.addEventListener('message', (event) => { if (event.data && event.data.type === 'NAVIGATE') { window.procesarEnlaceInterno(event.data.url); } }); }

const VAPID_KEY = "BO7AkZgMGzNtUBR8ZShudo6sW0zTbS7lyOZszkVrbJ3WLL80yEBRIfgreLnFpPHe4cBCLr_J8XmyckjpwMu6xTo";
window.registrarTokenPush = async function(codigoUsuario, modoSilencioso = false) { const btn = document.getElementById('btnActivarPushVip') || document.getElementById('btnActivarPushAdmin'); if (!("Notification" in window)) { if(!modoSilencioso) window.mostrarAlerta("iOS Incompatible", "Actualiza a iOS 16.4+ y añade la app a la pantalla de inicio.", "error"); if(btn) btn.remove(); return; } try { if(btn && !modoSilencioso) { btn.innerHTML = '<i class="fas fa-spinner fa-spin text-lg mr-2"></i> Solicitando permiso...'; } const permission = modoSilencioso ? Notification.permission : await Notification.requestPermission(); if (permission === 'granted') { if(btn && !modoSilencioso) { btn.innerHTML = '<i class="fas fa-spinner fa-spin text-lg mr-2"></i> Vinculando celular...'; } const swRegistration = await navigator.serviceWorker.ready; const tokenPromise = getToken(messaging, { vapidKey: VAPID_KEY, serviceWorkerRegistration: swRegistration }); const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error("Timeout")), 15000)); const token = await Promise.race([tokenPromise, timeoutPromise]); if (token) { let codigoGuardar = codigoUsuario || (adminAutenticado ? "ADMIN_MASTER" : "DESCONOCIDO"); if (codigoGuardar !== "DESCONOCIDO") { await setDoc(doc(db, "codigos_nube", codigoGuardar), { fcmToken: token }, { merge: true }); if(!modoSilencioso) window.mostrarAlerta("¡Fondo Vinculado!", "Recibirás señales Diamante directamente.", "success"); if(btn) { if(!modoSilencioso) { btn.innerHTML = '<i class="fas fa-check-circle text-lg mr-2"></i> Alertas Activadas'; btn.disabled = true; btn.classList.replace('bg-blue-600', 'bg-green-600'); } setTimeout(() => { btn.style.transition = "opacity 0.5s ease, height 0.5s ease, margin 0.5s ease, padding 0.5s ease"; btn.style.opacity = "0"; setTimeout(() => { btn.style.height = "0px"; btn.style.margin = "0px"; btn.style.padding = "0px"; btn.style.overflow = "hidden"; setTimeout(() => btn.remove(), 500); }, 500); }, modoSilencioso ? 0 : 1500); } } } else { if(!modoSilencioso) throw new Error("Token vacío"); } } else { if(!modoSilencioso) window.mostrarAlerta("Permiso Denegado", "Has bloqueado las alertas. Actívalas en la Configuración de tu celular.", "warning"); if(btn && !modoSilencioso) { btn.innerHTML = '<i class="fas fa-bell-slash text-lg mr-2"></i> Bloqueado'; } } } catch(e) { if(!modoSilencioso) window.mostrarAlerta("Fallo de Conexión", "Error: " + e.message, "error"); if(btn && !modoSilencioso) { btn.innerHTML = '<i class="fas fa-redo text-lg mr-2"></i> Reintentar Conexión'; } } };
...
window.verificarNotificacionesPendientes = async function() { if (!modoVipActivo && !adminAutenticado) return; try { const q = query(collection(db, "notificaciones_push"), orderBy("timestamp", "desc"), limit(1)); const snap = await getDocs(q); if (!snap.empty) { const data = snap.docs[0].data(); const esNotificacionEscalera = data.audiencia === "escalera" || (data.url && data.url.includes("escalera")) || (data.titulo && data.titulo.toLowerCase().includes("escalera")); if (esNotificacionEscalera && estadoEscalera !== "approved" && !adminAutenticado) { localStorage.setItem('oracle_last_push_seen', data.timestamp.toString()); return; } const ultimaVista = localStorage.getItem('oracle_last_push_seen'); if (!ultimaVista) { localStorage.setItem('oracle_last_push_seen', Date.now().toString()); return; } if (data.timestamp > parseInt(ultimaVista)) { localStorage.setItem('oracle_last_push_seen', data.timestamp.toString()); let urlDestino = null; if (data.url && data.url !== "/" && data.url.includes("view=")) { urlDestino = data.url; } window.mostrarAlerta(data.titulo, data.cuerpo, "info", urlDestino); } } } catch (error) { console.log("Error verificando bandeja oculta:", error); } };
...
if (!isStandalone) { if (isIOS) { const banner = document.getElementById('installBanner'); const btn = document.getElementById('btnInstalarApp'); if(banner) banner.classList.remove('hidden'); if(btn) { btn.onclick = () => { const m = document.getElementById('iosInstallModal'); if(m) { m.classList.remove('hidden'); m.style.display = 'flex'; } }; } } else { window.addEventListener('beforeinstallprompt', (e) => { e.preventDefault(); deferredPrompt = e; const banner = document.getElementById('installBanner'); const btn = document.getElementById('btnInstalarApp'); if(banner) banner.classList.remove('hidden'); if(btn) { btn.onclick = async () => { banner.classList.add('hidden'); deferredPrompt.prompt(); const { outcome } = await deferredPrompt.userChoice; if (outcome === 'accepted') { console.log('App instalada'); } deferredPrompt = null; }; } }); } }
...
// ==========================================
// 🚀 LÓGICA DE LOGIN MÁGICO (ADMIN INTEGRADO)
// ==========================================
...
window.preValidarCodigo = function() {
    ...
    // Si el usuario escribe un "@", la casilla se convierte en la de contraseña silenciosamente
    if (valRaw.includes('@')) {
        if (!correoAdminTemp) {
    ...
    // Si ya guardó el correo en la memoria, procede a loguearse como Admin sin mostrar Términos
    if (correoAdminTemp) {
        window.validarCodigo('INGRESAR ADMIN', document.getElementById('btnValidarCodigo'));
        return;
    }

    // Flujo normal VIP para clientes
...
window.validarCodigo = async function(txtOriginal = 'VERIFICAR ACCESO', btnObj = null) {
    ...
    // FLUJO DE ADMINISTRADOR: Firebase usa el correo guardado y la clave tipeada
    if (correoAdminTemp) {
        try {
...
// 🚀 CARTELERA MARGEN DE 4 HORAS EN EL FILTRO LOCAL
...
window.editarMiCapitalEscalera = function() {
    ...
    if(!isNaN(val) && val >= 1000) { localStorage.setItem('oracle_cap_escalera', val); window.cargarRetoEscaleraNube(); window.mostrarAlerta("Banca Actualizada", "Tu fondo inicial ha sido guardado en tu celular.", "success"); } else if (n !== null) { window.mostrarAlerta("Error", "Ingresa un número válido.", "error"); }
};
...
window.generarTicketApadrinamiento = async function() {
    ...
    setTimeout(async () => {
        ...
        if (esPlanB) {
            ...
            window.mostrarAlerta("Alerta de Riesgo", "No hay eventos VIP confirmados. Puedes forzar un ticket de Plan B reduciendo drásticamente tu apuesta.", "warning");
        } else {
...
window.renderizarSolicitudesAdmin = async function() { 
    ...
    try { const s = await getDocs(collection(db, "codigos_nube")); l.innerHTML = ''; let c = 0; s.forEach(d => { let da = d.data(); if(da.ladderStatus === 'pending') { c++; l.innerHTML += `<div class="flex justify-between items-center bg-black/60 p-3 rounded-lg border border-green-500/30 mb-2 shadow-md"><span class="text-white font-black text-[11px] uppercase"><i class="fas fa-user text-green-500 mr-2"></i>${da.code}</span><button onclick="window.aprobarEscalera('${da.code}')" class="bg-green-600 hover:bg-green-500 text-white px-4 py-2 rounded-md text-[9px] font-black uppercase transition-all active:scale-95 shadow-lg">Aprobar</button></div>`; } }); if(c === 0) { l.innerHTML = `<p class="text-[10px] text-gray-500 text-center py-4 font-bold uppercase tracking-widest border border-dashed border-white/10 rounded-lg">No hay solicitudes pendientes</p>`; } } catch(e){} 
};
...
// ==========================================
window.eliminarTicketAdmin = async function(idDoc, fromView = 'dash') { window.mostrarConfirmacion("Eliminar Ticket", "Se borrará permanentemente de la base de datos.", async () => { try { await deleteDoc(doc(db, "tickets_guardados", idDoc)); window.mostrarAlerta("Eliminado", "Ticket borrado con éxito.", "success"); if(fromView === 'dash') { window.cargarMonitorTickets(); } else if (fromView === 'users') { window.cargarUsuariosAdmin(true); window.cargarMonitorTickets(); } } catch(e) { window.mostrarAlerta("Error", "Fallo al intentar borrar.", "error"); } }); };
window.limpiarTodoMonitor = async function() { window.mostrarConfirmacion("Purgar Todo", "Esta acción eliminará TODO el historial de tickets guardados de todos los usuarios. Es irreversible. ¿Continuar?", async () => { try { const q = query(collection(db, "tickets_guardados")); const snap = await getDocs(q); let contador = 0; snap.forEach(async (d) => { await deleteDoc(d.ref); contador++; }); setTimeout(() => { window.cargarMonitorTickets(); window.cargarUsuariosAdmin(true); window.mostrarAlerta("Purga Completa", "Base de datos limpia.", "success"); }, 1500); } catch(e) { window.mostrarAlerta("Error", "No se pudo limpiar.", "error"); } }); };
...
window.verDetalleFondoAdmin = async function(codigo) {
    ...
    const statsContainer = document.getElementById('statsFondoAdmin'); const ticketsContainer = document.getElementById('ticketsFondoAdminContainer'); statsContainer.innerHTML = '<div class="col-span-2 text-center text-xs text-green-500"><i class="fas fa-spinner fa-spin"></i> Cargando...</div>'; ticketsContainer.innerHTML = '';
    try {
