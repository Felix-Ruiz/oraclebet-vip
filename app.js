...
// 🚀 SOLUCIÓN 1: PRECARGAR LA BASE DE DATOS PARA TODOS (INCLUYENDO AL ADMIN)
...
// 🚀 SOLUCIÓN 2: LOGIN DIRECTO CON CORREO ELECTRÓNICO
window.validarCodigo = async function(txtOriginal = 'VERIFICAR ACCESO', btnObj = null) {
    ...
    // Capturamos lo que escribió el usuario tal cual y le quitamos los espacios en blanco de los lados
    const codigoRaw = inputEl.value.trim();
    const codigoIngresado = codigoRaw.toUpperCase();
    ...
    // 🔥 NUEVO: Si escribes algo con "@", el sistema sabe que es un correo y asume que eres el Master
    if (codigoRaw.includes("@")) {
        try {
            let pw = prompt("Introduce tu contraseña de Administrador:");
            if (!pw) throw new Error("Cancelado");
            
            // Inicia sesión en Firebase con el correo en minúsculas y la contraseña sin espacios
            await promesaConTimeout(signInWithEmailAndPassword(auth, codigoRaw.toLowerCase(), pw.trim()), 8000);
    ...
    // Código original por si acaso alguna vez entras con MASTER_
    ...
    try {
        const docRef = doc(db, "codigos_nube", codigoIngresado); const docSnap = await promesaConTimeout(getDoc(docRef), 15000); // 🚀 Aumenté el tiempo de espera a 15 segundos
        if (docSnap.exists()) {
...
window.cargarHistorialEscaleraAdmin = async function() {
    ...
    try {
        ...
        if(snap.empty) { lista.innerHTML = `<p class="text-[10px] text-gray-500 text-center border border-dashed border-white/10 p-4 rounded-lg">No hay retos.</p>`; return; }
        snap.forEach(doc => {
