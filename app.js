                ...
                if (codigoGuardar !== "DESCONOCIDO") {
                    ...
                    if(btn) { 
                        ...
                        setTimeout(() => {
                            ...
                            btn.style.opacity = "0";
                            setTimeout(() => {
                                btn.style.height = "0px"; btn.style.margin = "0px"; btn.style.padding = "0px"; btn.style.overflow = "hidden";
    ...
    try {
        ...
        if (!snap.empty) {
            ...
            if (esNotificacionEscalera && estadoEscalera !== "approved" && !adminAutenticado) {
                localStorage.setItem('oracle_last_push_seen', data.timestamp.toString()); return; 
            }
            ...
            if (data.timestamp > parseInt(ultimaVista)) {
                ...
                let urlDestino = null;
                if (data.url && data.url !== "/" && data.url.includes("view=")) { urlDestino = data.url; }
                window.mostrarAlerta(data.titulo, data.cuerpo, "info", urlDestino);
            }
        }
    } catch (error) { console.log("Error verificando bandeja:", error); }
};
...
window.iniciarSwipeNotificaciones = function() {
    ...
    cards.forEach(card => {
        ...
        card.addEventListener('touchmove', e => {
            ...
            let currentX = e.touches[0].clientX; let diffX = startX - currentX;
            if (diffX > 10) { window.isSwiping = true; if (deleteBg) deleteBg.style.opacity = '1'; }
            if (diffX > 0 && diffX < 150) { card.style.transform = `translateX(-${diffX}px)`; }
        }, {passive: true});
        card.addEventListener('touchend', e => {
            isDragging = false; let diffX = startX - e.changedTouches[0].clientX; card.style.transition = 'transform 0.3s ease';
            if (diffX > 60) {
                card.style.transform = `translateX(-120%)`;
                const parent = card.closest('.notif-item'); const id = parent.dataset.id;
                let hidden = JSON.parse(localStorage.getItem('oracle_hidden_notifs') || '[]');
                if(!hidden.includes(id)) hidden.push(id); localStorage.setItem('oracle_hidden_notifs', JSON.stringify(hidden));
                setTimeout(() => {
                    parent.style.height = parent.offsetHeight + 'px'; parent.style.transition = 'all 0.3s ease'; parent.style.opacity = '0'; parent.style.height = '0px'; parent.style.marginBottom = '0px';
...
window.abrirBandejaNotificaciones = async function() {
    if (!modoVipActivo && !adminAutenticado) {
        window.mostrarAlerta("Acceso Restringido", "Debes iniciar sesión con tu credencial para leer los comunicados oficiales.", "warning");
    ...
    try {
        ...
        if(validCount === 0) { lista.innerHTML = `<div class="text-center mt-10 text-gray-500 text-xs font-bold uppercase tracking-widest"><i class="fas fa-check-circle text-4xl mb-3 opacity-50 block"></i> Bandeja Vacía</div>`; } 
        else { setTimeout(() => window.iniciarSwipeNotificaciones(), 50); }
...
window.cerrarModalLogin = function() { const m = document.getElementById('modalLogin'); if(m) { m.classList.add('hidden'); m.style.display = 'none'; } correoAdminTemp = ""; const inputElement = document.getElementById('vipCode'); const btn = document.getElementById('btnValidarCodigo'); if(inputElement) { inputElement.type = 'text'; inputElement.placeholder = 'CÓDIGO DE ACCESO'; inputElement.value = ''; } if(btn) { btn.innerHTML = 'VERIFICAR ACCESO'; } };
...
window.mostrarConfirmacion = function(titulo, mensaje, callback) {
    ...
    nuevoBtn.addEventListener('click', () => { window.cerrarConfirmGlobal(); callback(); }); 
    modal.classList.remove('hidden'); modal.style.display = 'flex';
};
...
if (!isStandalone) {
    if (isIOS) {
        ...
        window.addEventListener('beforeinstallprompt', (e) => {
            ...
            if(btn) {
                btn.onclick = async () => {
                    banner.classList.add('hidden'); deferredPrompt.prompt();
...
let CACHE_PARTIDOS_FUTUROS_VAR = []; let partidosGlobales_VAR = []; let partidosFiltrados_VAR = []; let competicionesGlobales_VAR = []; let seleccionesVIPGlobal_VAR = []; let ticketDinamicoVIP_VAR = []; let modoMercadoGlobal_VAR = 'mixto'; let modoRiesgoGlobal_VAR = false; 
let perfilApadrinamiento_VAR = null; let unsubscribeApadrinamiento_VAR = null; let tiempoInactividad_VAR = 0; const TIEMPO_MAXIMO_SEGUNDOS_VAR = 180; let timerInactividad_VAR; let filtroIAActivo_VAR = false; let correoAdminTemp_VAR = ""; 
            ...
            if(p.sport_key && p.sport_key.includes('soccer')) { 
                ...
                if(p.bookmakers) { 
                    p.bookmakers.forEach(b => { 
                        b.markets?.forEach(m => { 
                            m.outcomes?.forEach(o => { if (o.verificado_ia) tieneIA = true; }); 
                        }); 
                    }); 
                }
                p._tieneIA = tieneIA;
...
const promesaConTimeout = (promesa, ms) => { 
    let timeout = new Promise((resolve, reject) => { let id = setTimeout(() => { clearTimeout(id); reject(new Error("Timeout")); }, ms); }); 
...
// 🚀 REPARACIÓN: LOGIN ADMIN Y DESTRUCCIÓN DE ESPACIOS
window.validarCodigo = async function(txtOriginal = 'VERIFICAR ACCESO', btnObj = null) {
    ...
    if (codigoIngresado.startsWith("MASTER_")) {
        try {
            if (!correoAdminTemp) {
                ...
                correoAdminTemp = pmt.trim(); // 🚀 Esto arregla el error de Firebase eliminando los espacios
            }
    ...
    try {
        ...
        if (docSnap.exists()) {
            ...
            if (data.deviceID && data.deviceID !== HUELLA_ESTE_CELULAR) {
                window.mostrarAlerta("Licencia en Uso", "Este código ya está vinculado a otro celular.", "error");
            } else {
    ...
    } catch(e) { window.mostrarAlerta("Error de Red", "Tiempo de espera agotado.", "error"); } finally { if(btn) { btn.innerHTML = txtOriginal; btn.disabled = false; } }
};
...
window.enviarNotificacionGlobal = async function() {
    ...
    try {
        await setDoc(doc(collection(db, "notificaciones_push")), { titulo: titulo, cuerpo: cuerpo, url: window.location.origin + "/?inbox=true", timestamp: Date.now(), enviadoPor: "FR (Gestor)", audiencia: "todos" });
        window.mostrarAlerta("Éxito", "La notificación ha sido enviada.", "success");
    ...
    } catch(e) { window.mostrarAlerta("Error", "No se pudo comunicar con el servidor.", "error"); } finally { btn.innerHTML = `<i class="fas fa-paper-plane mr-1"></i> Notificar a Inversores`; btn.disabled = false; }
}
...
window.cargarNotificacionesAdmin = async function() {
    ...
    try {
        ...
        snap.forEach(doc => {
            let data = doc.data(); let f = new Date(data.timestamp).toLocaleDateString('es-CO', { month: 'short', day: '2-digit', hour: '2-digit', minute: '2-digit' });
...
window.eliminarNotificacionAdmin = async function(idDoc) { window.mostrarConfirmacion("Eliminar Comunicado", "¿Deseas borrar este mensaje?", async () => { try { await deleteDoc(doc(db, "notificaciones_push", idDoc)); window.mostrarAlerta("Eliminada", "Notificación borrada.", "success"); window.cargarNotificacionesAdmin(); } catch(e) { window.mostrarAlerta("Error", "No se pudo borrar.", "error"); } }); };
...
window.renderizarLayoutAdmin = function() {
    ...
    window.cargarMonitorTickets(); window.cargarUsuariosAdmin(); window.cargarFondosAdmin(); window.renderizarListaAdmin(); window.renderizarSolicitudesAdmin(); window.cargarHistorialEscaleraAdmin(); window.cargarNotificacionesAdmin(); window.cargarGestionRetoActivoAdmin();
...
window.cambiarTabAdmin = function(tabName) { document.querySelectorAll('.admin-view-content').forEach(el => el.classList.add('hidden')); document.querySelectorAll('.admin-tab-btn').forEach(btn => { btn.classList.remove('bg-yellow-500', 'text-black', 'shadow-md'); btn.classList.add('text-gray-400'); }); document.getElementById('vistaAdm_' + tabName).classList.remove('hidden'); let activeBtn = document.getElementById('tab_' + tabName); activeBtn.classList.remove('text-gray-400'); activeBtn.classList.add('bg-yellow-500', 'text-black', 'shadow-md'); };
...
window.aplicarFiltrosLocales = function() {
    const buscador = document.getElementById('buscadorEquipos'); const filtroF = document.getElementById('filtroFecha'); const texto = buscador ? buscador.value.toLowerCase() : ""; const fechaFiltro = filtroF ? filtroF.value : ""; const ahoraTS = Date.now(); 
    partidosFiltrados = partidosGlobales.filter(p => { if (p._timestamp < ahoraTS) return false; if (texto && !p._textoBusqueda.includes(texto)) return false; if (fechaFiltro && p._fechaFiltro !== fechaFiltro) return false; if (filtroIAActivo && !p._tieneIA) return false; return true; });
...
window.renderizarPartidosVIP = function() {
    ...
    for(const [nombrePais, dataPais] of Object.entries(paisesAgrupados)) {
        ...
        for(const [nombreLiga, partidos] of Object.entries(dataPais.ligas)) {
            ...
            partidos.forEach(p => { 
                const d = new Date(p.commence_time); const f = d.toLocaleDateString('es-ES', {day: '2-digit', month: 'short'}) + ' • ' + d.toLocaleTimeString('es-ES', {hour: '2-digit', minute:'2-digit'}); const sel = seleccionesVIPGlobal.some(s => s.id === p.id); let cardClasses = sel ? 'border-yellow-500/50 bg-yellow-500/5 shadow-[0_0_10px_rgba(212,175,55,0.1)]' : 'border-white/5'; let checkAttr = sel ? 'checked' : ''; 
...
window.guardarTicketHistorial = async function(cuota) {
    ...
    let picksObj = ticketDinamicoVIP.map(t => { const idxUsado = modoRiesgoGlobal ? t.indexRiesgo : t.indexSeleccionado; const o = t.opciones[idxUsado]; return { partido: `${t.partido.home_team} vs ${t.partido.away_team}`, mercadoKey: o.mercadoKey, pick: formatearPickEspanol(o.nombre, o.point, o.mercadoKey), cuota: o.cuota, probabilidad: o.probabilidad, verificado_ia: o.verificado_ia, es_valor: o.es_valor, edgeValor: o.edgeValor }; });
    try { 
        let hist = JSON.parse(localStorage.getItem('oracle_historial_' + codigoActivoUsuario)) || []; let nuevoTicket = { id: Date.now(), fecha: new Date().toLocaleDateString('es-CO'), cuota: cuota, picksObj: picksObj, estado: 'pendiente' }; hist.unshift(nuevoTicket); localStorage.setItem('oracle_historial_' + codigoActivoUsuario, JSON.stringify(hist)); 
            ...
            hist.forEach(t => {
                ...
                if(t.picksObj) { 
                    picksHtml = t.picksObj.map(p => { 
                        const defMercado = definicionesApuestas[p.mercadoKey] || { 'titulo': 'Mercado Especial' }; let ico = "fa-handshake"; if(p.mercadoKey.includes('shots')) ico = "fa-bullseye"; else if(p.mercadoKey.includes('corners')) ico = "fa-flag"; else if(p.mercadoKey.includes('cards')) ico = "fa-square"; else if(p.mercadoKey === 'totals') ico = "fa-futbol"; else if(p.mercadoKey === 'spreads') ico = "fa-balance-scale"; 
                        let badgeIA = p.verificado_ia ? `<span class="bg-blue-600 text-white px-1.5 py-0.5 rounded text-[7px] font-black uppercase ml-2 shadow-sm"><i class="fas fa-robot"></i> IA</span>` : ''; let probBadge = p.probabilidad ? `<div class="absolute top-0 right-0 bg-gray-700 text-white text-[8px] font-black px-2 py-1 rounded-bl-lg shadow-md">PROB: ${p.probabilidad}%</div>` : '';
...
window.chequearApadrinamientoUI = function() {
    const b = document.getElementById('apadrinamientoBloqueado'); const o = document.getElementById('apadrinamientoOnboarding'); const d = document.getElementById('apadrinamientoDashboard'); if(!b || !o || !d) return;
    if(!codigoActivoUsuario) { b.classList.remove('hidden'); o.classList.add('hidden'); d.classList.add('hidden'); return; } b.classList.add('hidden');
    if (!perfilApadrinamiento || !perfilApadrinamiento.activo) { o.classList.remove('hidden'); d.classList.add('hidden'); } else { o.classList.add('hidden'); d.classList.remove('hidden'); window.renderizarDashboardApadrinamiento(); }
};

window.suscribirApadrinamiento = function() { if(!codigoActivoUsuario) return; if(unsubscribeApadrinamiento) unsubscribeApadrinamiento(); try { unsubscribeApadrinamiento = onSnapshot(doc(db, "codigos_nube", codigoActivoUsuario), (s) => { if(s.exists()) { const d = s.data(); perfilApadrinamiento = d.apadrinamiento || null; estadoEscalera = d.ladderStatus || 'none'; if(document.getElementById('vista_escalera')?.classList.contains('view-active')) { window.chequearEstadoEscaleraUI(); } if(document.getElementById('vista_apadrinamiento')?.classList.contains('view-active')) { window.chequearApadrinamientoUI(); } } }); } catch(e) {} };

window.iniciarApadrinamiento = async function() {
    const c = document.getElementById('checkTerminosApadrinamiento');
    if (c && !c.checked) { return window.mostrarAlerta("Atención", "Debes leer y aceptar el Acuerdo para poder utilizar la terminal.", "error"); }
    const v = document.getElementById('inputBankrollInicial').value; const m = parseFloat(v);
    if (!m || m < 10000) { return window.mostrarAlerta("Error", "Ingresa un bankroll válido.", "error"); }
    let uA = typeof codigoActivoUsuario !== 'undefined' && codigoActivoUsuario ? codigoActivoUsuario : localStorage.getItem('oracle_vip_code');
    if (!uA) { return window.mostrarAlerta("Sesión Expirada", "Por favor inicia sesión.", "error"); }
    const b = document.querySelector('#apadrinamientoOnboarding button'); const tO = b ? b.innerText : 'ACTIVAR TERMINAL';
    if(b) { b.innerText = "PROCESANDO..."; b.disabled = true; }
    try {
        await setDoc(doc(db, "codigos_nube", uA), { apadrinamiento: { bankroll_inicial: m, bankroll_actual: m, fecha_inicio: Date.now(), activo: true } }, { merge: true });
        window.mostrarAlerta("Éxito", "Software configurado.", "success");
        const o = document.getElementById('apadrinamientoOnboarding'); const d = document.getElementById('apadrinamientoDashboard');
        if(o) o.style.display = 'none'; if(d) { d.style.display = 'block'; d.classList.remove('hidden'); }
        if(window.cargarDatosApadrinamiento) window.cargarDatosApadrinamiento();
    } catch(e) { window.mostrarAlerta("Error", e.message, "error"); } finally { if(b) { b.innerText = tO; b.disabled = false; } }
};

window.abrirModalTerminosApadrinamiento = function() { const m = document.getElementById('modalTerminosApadrinamiento'); if(m) { m.classList.remove('hidden'); m.style.display = 'flex'; } };
window.cerrarModalTerminosApadrinamiento = function() { const m = document.getElementById('modalTerminosApadrinamiento'); if(m) { m.classList.add('hidden'); m.style.display = 'none'; } };
...
window.renderizarDashboardApadrinamiento = async function() {
    ...
    let r = ((perfilApadrinamiento.bankroll_actual - perfilApadrinamiento.bankroll_inicial) / perfilApadrinamiento.bankroll_inicial) * 100; let uR = document.getElementById('uiBankrollRendimiento'); let uB = document.getElementById('uiBankrollBadge');
    if (r > 0) { uR.innerText = `Rendimiento: +${r.toFixed(2)}%`; uR.className = "text-[10px] font-bold text-green-400 mt-1"; uB.className = "absolute top-0 right-0 bg-green-600 text-white text-[8px] font-black px-2 py-1 rounded-bl-lg"; } else if (r < 0) { uR.innerText = `Rendimiento: ${r.toFixed(2)}%`; uR.className = "text-[10px] font-bold text-red-400 mt-1"; uB.className = "absolute top-0 right-0 bg-red-600 text-white text-[8px] font-black px-2 py-1 rounded-bl-lg"; } else { uR.innerText = `Rendimiento: 0.00%`; uR.className = "text-[10px] font-bold text-gray-400 mt-1"; uB.className = "absolute top-0 right-0 bg-gray-600 text-white text-[8px] font-black px-2 py-1 rounded-bl-lg"; }
    let pE = document.getElementById('previewArriesgar'); if(pE) pE.remove();
    const h = new Date().toLocaleDateString('es-CO', {timeZone: 'America/Bogota'}); const b = document.getElementById('btnGenerarApadrinamiento');
    if (perfilApadrinamiento.ultimo_dia_generado === h) { b.innerHTML = `<i class="fas fa-check-circle text-lg"></i> OPERACIÓN LISTA`; b.className = "w-full py-4 bg-gray-800 text-gray-500 rounded-xl font-black text-[12px] uppercase cursor-not-allowed border border-gray-700"; b.disabled = true; } else { b.innerHTML = `<i class="fas fa-search-dollar text-lg"></i> BUSCAR OPORTUNIDAD`; b.className = "w-full py-4 bg-gradient-to-r from-yellow-600 to-yellow-500 text-black rounded-xl font-black text-[12px] uppercase shadow-[0_10px_20px_rgba(212,175,55,0.3)] transition-all active:scale-95"; b.disabled = false; } window.cargarHistorialApadrinamiento();
};

window.generarTicketApadrinamiento = async function() {
    if(!perfilApadrinamiento) return; const b = document.getElementById('btnGenerarApadrinamiento'); b.innerHTML = `<i class="fas fa-spinner fa-spin text-lg"></i> ESCANEANDO...`; b.disabled = true;
    let pE = document.getElementById('previewArriesgar'); if(pE) pE.remove();

    setTimeout(async () => {
        const a = new Date(); const lF = new Date(); lF.setDate(a.getDate() + 2); lF.setHours(23, 59, 59, 999);
        let pR = CACHE_PARTIDOS_FUTUROS.filter(p => { const d = new Date(p.commence_time); return d >= a && d <= lF; });
        let oD = []; let oR = []; pR.forEach(p => { let o = obtenerOpcionesRentables(p); o.forEach(oc => { if(oc.verificado_ia && oc.es_valor) oD.push({ partido: p, opcion: oc }); if(oc.probabilidad >= 60) oR.push({ partido: p, opcion: oc }); }); }); oD.sort((x, y) => y.opcion.edgeValor - x.opcion.edgeValor); oR.sort((x, y) => y.opcion.probabilidad - x.opcion.probabilidad);
        let tF = []; let cA = 1.0; let pP = 0; let ePB = false; let pUD = oD.find(x => x.opcion.cuota >= 1.30 && x.opcion.cuota <= 1.55);
        if (pUD) { tF.push(pUD); cA = pUD.opcion.cuota; pP = pUD.opcion.probabilidad; } else { let en = false; for(let i = 0; i < oD.length && !en; i++) { for(let j = i + 1; j < oD.length && !en; j++) { if (oD[i].partido.id !== oD[j].partido.id) { let co = oD[i].opcion.cuota * oD[j].opcion.cuota; if (co >= 1.30 && co <= 1.60) { tF.push(oD[i]); tF.push(oD[j]); cA = co; pP = (oD[i].opcion.probabilidad + oD[j].opcion.probabilidad) / 2; en = true; } } } } }
        if (tF.length === 0) { ePB = true; let pUR = oR.find(x => x.opcion.cuota >= 1.30 && x.opcion.cuota <= 1.55); if (pUR) { tF.push(pUR); cA = pUR.opcion.cuota; pP = pUR.opcion.probabilidad; } else { let eR = false; for(let i = 0; i < oR.length && !eR; i++) { for(let j = i + 1; j < oR.length && !eR; j++) { if (oR[i].partido.id !== oR[j].partido.id) { let co = oR[i].opcion.cuota * oR[j].opcion.cuota; if (co >= 1.30 && co <= 1.60) { tF.push(oR[i]); tF.push(oR[j]); cA = co; pP = (oR[i].opcion.probabilidad + oR[j].opcion.probabilidad) / 2; eR = true; } } } } } }

        if (tF.length === 0) { window.mostrarAlerta("Mercado Inestable", "El mercado está seco.", "error"); b.innerHTML = `<i class="fas fa-shield-alt text-lg"></i> SIN OPCIONES (72H)`; return; }
        
        let pS = 10; if (ePB) { pS = Math.max(1, Math.min(5, Math.floor((pP - 50) / 9))); }
        const mA = Math.floor(perfilApadrinamiento.bankroll_actual * (pS / 100)); const hS = new Date().toLocaleDateString('es-CO', {timeZone: 'America/Bogota'}); const tI = Date.now().toString();
        let oT = { id: tI, codigo_usuario: codigoActivoUsuario, fecha: hS, cuota_sistema: parseFloat(cA.toFixed(2)), cuota_usuario: parseFloat(cA.toFixed(2)), stake_porcentaje: pS, monto_apostar: mA, estado: 'pendiente', timestamp: Date.now(), picks: tF.map(t => { const d = new Date(t.partido.commence_time); const fS = d.toLocaleDateString('es-ES', {day: '2-digit', month: 'short', timeZone: 'America/Bogota'}); return { partido: `[${fS}] ${t.partido.home_team} vs ${t.partido.away_team}`, mercado: t.opcion.mercadoKey, pick: formatearPickEspanol(t.opcion.nombre, t.opcion.point, t.opcion.mercadoKey), cuota: t.opcion.cuota, probabilidad: t.opcion.probabilidad, verificado_ia: t.opcion.verificado_ia }; }) };

        if (ePB) {
            let pH = tF.map(t => { const d = new Date(t.partido.commence_time); const fS = d.toLocaleDateString('es-ES', {day: '2-digit', month: 'short', timeZone: 'America/Bogota'}); const dM = definicionesApuestas[t.opcion.mercadoKey] || { 'titulo': 'Mercado Especial' }; let ic = "fa-handshake"; if(t.opcion.mercadoKey.includes('shots')) ic = "fa-bullseye"; else if(t.opcion.mercadoKey.includes('corners')) ic = "fa-flag"; return `<div class="bg-gray-900/50 p-3 rounded-xl mb-2 border border-white/5 relative overflow-hidden shadow-md"><div class="absolute top-0 right-0 bg-blue-600 text-white text-[8px] font-black px-2 py-1 rounded-bl-lg shadow-md">PROB: ${t.opcion.probabilidad}%</div><div class="text-[8px] text-gray-400 font-bold uppercase mb-1"><i class="fas ${ic} mr-1"></i> ${dM.titulo} • <span class="text-yellow-500">${fS}</span></div><div class="text-[10px] font-bold text-white mb-2 border-b border-white/5 pb-2">${t.partido.home_team} vs ${t.partido.away_team}</div><div class="flex justify-between items-center bg-black/60 p-2 rounded-lg border border-gray-700"><span class="text-[10px] text-yellow-500 font-black uppercase tracking-wide">PICK: ${formatearPickEspanol(t.opcion.nombre, t.opcion.point, t.opcion.mercadoKey)}</span><span class="text-white font-black text-sm">${t.opcion.cuota.toFixed(2)}</span></div></div>`; }).join('');
            let c = document.createElement('div'); c.id = 'previewArriesgar'; b.parentNode.insertBefore(c, b);
            c.innerHTML = `<div class="bg-red-900/20 border border-red-500/50 p-4 rounded-xl mb-3 shadow-inner transform transition-all animate-pulse"><div class="text-red-400 font-black text-[11px] uppercase mb-2"><i class="fas fa-exclamation-triangle"></i> MERCADO INESTABLE - PLAN B</div><p class="text-[9px] text-gray-300 mb-3 leading-relaxed">FR reducirá tu Stake al <b class="text-yellow-500 text-xs">${pS}%</b> para proteger tu capital.</p><div class="mb-3">${pH}</div><div class="text-right text-xs font-black text-white pt-2 border-t border-white/10">Cuota Final: <span class="text-yellow-500">${cA.toFixed(2)}</span></div></div>`;
            window.ticketPlanBPendiente = oT; b.removeAttribute('onclick'); b.innerHTML = `<i class="fas fa-fire"></i> ARRIESGAR STAKE (${pS}%)`; b.className = "w-full py-4 bg-gradient-to-r from-red-600 to-red-700 hover:from-red-500 hover:to-red-600 text-white rounded-xl font-black text-[12px] uppercase shadow-[0_10px_20px_rgba(220,38,38,0.3)] transition-all active:scale-95"; b.disabled = false;
            b.onclick = async function() { b.innerHTML = `<i class="fas fa-spinner fa-spin text-lg"></i> ENVIANDO...`; b.disabled = true; try { await setDoc(doc(db, "tickets_apadrinamiento", tI), window.ticketPlanBPendiente); await updateDoc(doc(db, "codigos_nube", codigoActivoUsuario), { "apadrinamiento.ultimo_dia_generado": hS }); window.mostrarAlerta("Riesgo Asumido", `Ticket guardado con Stake de protección (${pS}%).`, "warning"); window.renderizarDashboardApadrinamiento(); } catch(e) { window.mostrarAlerta("Error", "Error al procesar.", "error"); window.renderizarDashboardApadrinamiento(); } };
            window.mostrarAlerta("Alerta de Riesgo", "Tienes la opción de forzar un Plan B.", "warning");
        } else {
            try { await setDoc(doc(db, "tickets_apadrinamiento", tI), oT); await updateDoc(doc(db, "codigos_nube", codigoActivoUsuario), { "apadrinamiento.ultimo_dia_generado": hS }); window.mostrarAlerta("Oportunidad Encontrada", `Operación Diamante guardada con Stake del 10%.`, "success"); window.renderizarDashboardApadrinamiento(); } catch(e) { window.mostrarAlerta("Error", "No se pudo guardar.", "error"); window.renderizarDashboardApadrinamiento(); }
        }
    }, 1500); 
};

window.cargarHistorialApadrinamiento = async function() {
    const c = document.getElementById('contenedorTicketsApadrinamiento'); if(!c) return; c.innerHTML = `<p class="text-center text-xs text-gray-500"><i class="fas fa-spinner fa-spin"></i> Cargando...</p>`;
    try {
        const q = query(collection(db, "tickets_apadrinamiento"), where("codigo_usuario", "==", codigoActivoUsuario)); const s = await getDocs(q); c.innerHTML = '';
        if(s.empty) { c.innerHTML = `<p class="text-[10px] text-gray-500 text-center border border-white/5 p-4 rounded-lg bg-black/30">Sin historial.</p>`; return; }
        let tA = []; s.forEach(d => tA.push(d.data())); tA.sort((x, y) => y.timestamp - x.timestamp);
        tA.forEach(t => {
            let bS = 'bg-gray-800/50 border-gray-600'; let bSt = '<span class="bg-gray-700 text-gray-300 px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest shadow-sm"><i class="far fa-clock"></i> Abierta</span>';
            if(t.estado === 'won') { bS = 'bg-green-900/20 border-green-500/50 shadow-[0_0_15px_rgba(34,197,94,0.1)]'; bSt = '<span class="bg-green-600 text-white px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest shadow-md"><i class="fas fa-check-circle"></i> Ganada</span>'; }
            if(t.estado === 'lost') { bS = 'bg-red-900/20 border-red-500/50 shadow-[0_0_15px_rgba(239,68,68,0.1)]'; bSt = '<span class="bg-red-600 text-white px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest shadow-md"><i class="fas fa-times-circle"></i> Perdida</span>'; }
            let pH = t.picks.map(p => { const dM = definicionesApuestas[p.mercado] || { 'titulo': 'Mercado Especial' }; let ic = "fa-handshake"; if(p.mercado.includes('shots')) ic = "fa-bullseye"; else if(p.mercado.includes('corners')) ic = "fa-flag"; let bIA = p.verificado_ia ? `<span class="bg-blue-600 text-white px-1.5 py-0.5 rounded text-[7px] font-black uppercase ml-2 shadow-sm"><i class="fas fa-robot"></i> IA</span>` : ''; let pB = p.probabilidad ? `<div class="absolute top-0 right-0 bg-gray-700 text-white text-[8px] font-black px-2 py-1 rounded-bl-lg shadow-md">PROB: ${p.probabilidad}%</div>` : ''; return `<div class="bg-gray-900/60 p-3 rounded-lg mb-2 border border-white/5 relative overflow-hidden shadow-inner">${pB}<div class="text-[8px] text-gray-400 font-bold uppercase mb-1"><i class="fas ${ic} mr-1"></i> ${dM.titulo}</div><div class="text-[10px] font-bold text-white mb-2 border-b border-white/5 pb-1">${p.partido}</div><div class="flex justify-between items-center bg-black/40 p-2 rounded border border-gray-700"><div class="flex items-center gap-1.5"><span class="text-[9px] text-yellow-500 font-black uppercase tracking-wide">PICK: ${p.pick}</span>${bIA}<button onclick="window.abrirModalAyuda('${p.mercado}', '${p.pick.replace(/'/g, "\\'")}')" class="text-gray-500 hover:text-yellow-500 transition-colors text-xs p-0.5"><i class="fas fa-question-circle"></i></button></div><span class="text-white font-black text-[11px]">${p.cuota ? parseFloat(p.cuota).toFixed(2) : ''}</span></div></div>`; }).join('');
            c.innerHTML += `<div class="${bS} p-4 rounded-xl border relative mb-4 transition-all"><div class="flex justify-between items-center mb-3 border-b border-white/10 pb-2"><span class="text-[9px] font-bold text-gray-400"><i class="far fa-calendar-alt"></i> Generado: ${t.fecha}</span>${bSt}</div><div class="mb-4">${pH}</div><div class="flex justify-between items-end bg-black/60 p-3 rounded-lg border border-black shadow-inner"><div class="flex flex-col gap-1 w-1/2 border-r border-white/10 pr-3"><span class="text-[8px] text-gray-500 uppercase font-bold">Inversión (${t.stake_porcentaje}%)</span><span class="text-sm font-black text-white">${formatoCOP(t.monto_apostar)}</span></div><div class="flex flex-col gap-1 w-1/2 pl-3"><span class="text-[8px] text-gray-500 uppercase font-bold flex justify-between items-center">Cuota Real ${t.estado === 'pendiente' ? `<button onclick="window.editarCuotaUsuario('${t.id}', ${t.cuota_usuario})" class="text-blue-400 hover:text-blue-300 p-1"><i class="fas fa-edit"></i></button>` : ''}</span><span class="text-sm font-black text-yellow-500">${t.cuota_usuario.toFixed(2)}</span></div></div></div>`;
        });
    } catch(e) { c.innerHTML = `<p class="text-red-500 text-[10px] text-center">Error al cargar historial.</p>`; }
};

window.editarCuotaUsuario = async function(id, cA) { const n = prompt("Ingresa la cuota exacta (ej: 1.45):", cA); if (n && !isNaN(parseFloat(n))) { try { await updateDoc(doc(db, "tickets_apadrinamiento", id), { cuota_usuario: parseFloat(n) }); window.cargarHistorialApadrinamiento(); window.mostrarAlerta("Guardado", "Estadística actualizada.", "success"); } catch(e) {} } };

// ==========================================
// 12. ADMIN: RETO ESCALERA
// ==========================================
window.renderizarLayoutAdmin = function() {
    window.cerrarModalLogin(); document.querySelectorAll('.view-section').forEach(el => el.style.display = 'none');
    const bN = document.getElementById('bottomNav'); const fA = document.getElementById('footerApp'); if(bN) bN.style.display = 'none'; if(fA) fA.style.display = 'none'; 
    let bT = document.getElementById('btnTopLogin'); if(bT) { const nB = bT.cloneNode(true); bT.parentNode.replaceChild(nB, bT); nB.innerHTML = '<i class="fas fa-sign-out-alt"></i> <span>SALIR</span>'; nB.className = "shrink-0 bg-gray-800 text-red-500 text-[9px] px-3 py-2 rounded-full font-black border border-red-500/50 flex items-center gap-1.5 transition-all"; nB.onclick = async () => { await signOut(auth); location.reload(); }; }
    const aS = document.getElementById('adminSection'); if(!aS) return; aS.style.display = 'block';
    let bPA = ''; if (window.Notification && Notification.permission !== 'denied') { bPA = `<button id="btnActivarPushAdmin" onclick="window.registrarTokenPush('ADMIN_MASTER')" class="w-full bg-blue-600 text-white py-4 rounded-xl mb-5 text-[12px] font-black uppercase tracking-widest shadow-[0_10px_20px_rgba(37,99,235,0.3)] active:scale-95 transition-transform"><i class="fas fa-bell mr-2 animate-bounce"></i> Activar Alertas Master</button>`; }
    aS.innerHTML = `<div class="p-4 bg-gray-900 min-h-screen pb-20"><div class="flex flex-col items-center justify-center gap-2 mb-6 text-center border-b border-white/5 pb-4 relative"><div class="bg-gray-800 p-4 rounded-full text-yellow-500 shadow-inner mt-4"><i class="fas fa-chart-line text-2xl"></i></div><h2 class="text-yellow-500 font-black text-2xl uppercase tracking-widest flex items-center gap-2">FR</h2><h3 class="text-gray-400 text-[10px] font-bold uppercase tracking-[0.3em]">| Analytics |</h3><span class="bg-gray-700 text-white text-[8px] px-2 py-0.5 rounded-full mt-2 font-black tracking-widest">CRM ADMIN</span></div><div class="grid grid-cols-5 gap-1 mb-5 bg-black/60 p-1 rounded-xl border border-white/5"><button onclick="window.cambiarTabAdmin('dash')" id="tab_dash" class="admin-tab-btn py-2 rounded-lg text-[8px] font-black uppercase transition-all bg-yellow-500 text-black shadow-md">General</button><button onclick="window.cambiarTabAdmin('users')" id="tab_users" class="admin-tab-btn py-2 rounded-lg text-[8px] font-black uppercase transition-all text-gray-400 hover:text-white">Tickets</button><button onclick="window.cambiarTabAdmin('fondo')" id="tab_fondo" class="admin-tab-btn py-2 rounded-lg text-[8px] font-black uppercase transition-all text-gray-400 hover:text-white">Fondos</button><button onclick="window.cambiarTabAdmin('ladder')" id="tab_ladder" class="admin-tab-btn py-2 rounded-lg text-[8px] font-black uppercase transition-all text-gray-400 hover:text-white">Escalera</button><button onclick="window.cambiarTabAdmin('access')" id="tab_access" class="admin-tab-btn py-2 rounded-lg text-[8px] font-black uppercase transition-all text-gray-400 hover:text-white">Accesos</button></div><div id="vistaAdm_dash" class="admin-view-content block">${bPA}<div class="bg-gradient-to-r from-blue-900/40 to-blue-800/20 border border-blue-500/50 p-4 rounded-xl mb-5 shadow-lg relative overflow-hidden"><div class="absolute top-0 right-0 bg-blue-600 text-white text-[8px] font-black px-2 py-1 rounded-bl-lg shadow-md">LIVE</div><h3 class="text-[11px] font-black text-blue-400 uppercase tracking-widest mb-3 flex items-center"><i class="fas fa-bullhorn mr-2 text-lg"></i> Megáfono Inversores</h3><input type="text" id="pushTitulo" placeholder="Ej: Nueva Señal Diamante" class="w-full bg-black/50 border border-blue-500/30 rounded-lg p-3 text-white text-xs outline-none focus:border-blue-400 mb-2 font-bold shadow-inner"><textarea id="pushCuerpo" rows="2" placeholder="Escribe tu mensaje a todos los clientes..." class="w-full bg-black/50 border border-blue-500/30 rounded-lg p-3 text-white text-xs outline-none focus:border-blue-400 mb-3 shadow-inner resize-none"></textarea><button id="btnEnviarPush" onclick="window.enviarNotificacionGlobal()" class="w-full bg-blue-600 hover:bg-blue-500 text-white py-3.5 rounded-lg text-[10px] font-black uppercase transition active:scale-95 shadow-lg shadow-blue-500/30"><i class="fas fa-paper-plane mr-1"></i> Notificar a Inversores</button></div><div class="flex justify-between items-center mb-3 border-t border-white/5 pt-4"><h3 class="text-[11px] font-black text-white uppercase tracking-widest"><i class="fas fa-history text-blue-500 mr-1"></i> Historial de Comunicados</h3><button onclick="window.cargarNotificacionesAdmin()" class="text-gray-500 hover:text-white p-1"><i class="fas fa-sync-alt"></i></button></div><div id="adminNotificacionesList" class="space-y-2 mb-6"></div><div class="flex justify-between items-center mb-3 border-t border-white/5 pt-4"><h3 class="text-[11px] font-black text-white uppercase tracking-widest"><i class="fas fa-globe text-yellow-500 mr-1"></i> Últimos Globales</h3><button onclick="window.limpiarTodoMonitor()" class="bg-red-900/30 border border-red-500/50 hover:bg-red-900/60 text-red-400 px-3 py-1.5 rounded-lg text-[8px] font-black uppercase transition active:scale-95 shadow-sm"><i class="fas fa-dumpster-fire mr-1"></i> Purgar Todo</button></div><div id="monitorTicketsList" class="space-y-3"></div></div><div id="vistaAdm_users" class="admin-view-content hidden"><div id="panelListaUsuariosAdmin" class="block"><div class="relative mb-4"><i class="fas fa-search absolute left-4 top-3.5 text-gray-500"></i><input type="text" id="buscadorAdminUsuarios" onkeyup="window.filtrarUsuariosAdmin()" placeholder="Buscar código VIP..." class="w-full bg-black/60 border border-white/10 py-3 pl-10 pr-4 rounded-xl text-white text-xs outline-none focus:border-yellow-500 transition-colors shadow-inner"></div><div id="listaUsuariosAdminContainer" class="space-y-2"></div></div><div id="panelDetalleUsuarioAdmin" class="hidden"><button onclick="window.volverAusuariosAdmin()" class="mb-4 bg-gray-800 text-yellow-500 text-[10px] font-black uppercase px-4 py-2 rounded-lg border border-white/5 shadow-md active:scale-95 transition-transform"><i class="fas fa-arrow-left mr-1"></i> Volver al listado</button><h3 id="tituloDetalleUsuarioAdmin" class="text-xs font-black text-white mb-4 uppercase tracking-widest border-b border-white/10 pb-2"></h3><div id="ticketsUsuarioAdminContainer" class="space-y-3"></div></div></div><div id="vistaAdm_fondo" class="admin-view-content hidden"><div id="panelListaFondoAdmin" class="block"><h3 class="text-[11px] font-black text-green-400 uppercase tracking-widest mb-4 border-l-2 border-green-500 pl-2">Inversores Activos</h3><div id="listaFondoAdminContainer" class="space-y-2"></div></div><div id="panelDetalleFondoAdmin" class="hidden"><button onclick="window.volverAfondoAdmin()" class="mb-4 bg-gray-800 text-green-500 text-[10px] font-black uppercase px-4 py-2 rounded-lg border border-white/5 shadow-md active:scale-95 transition-transform"><i class="fas fa-arrow-left mr-1"></i> Volver a Inversores</button><h3 id="tituloDetalleFondoAdmin" class="text-xs font-black text-white mb-2 uppercase tracking-widest"></h3><div id="statsFondoAdmin" class="grid grid-cols-2 gap-2 mb-4 border-b border-white/10 pb-4"></div><div id="ticketsFondoAdminContainer" class="space-y-3"></div></div></div><div id="vistaAdm_ladder" class="admin-view-content hidden space-y-4"><div class="bg-black/60 p-4 rounded-2xl border border-white/5 shadow-md"><div class="flex justify-between items-center mb-3"><h3 class="text-[10px] text-gray-400 font-bold uppercase"><i class="fas fa-hand-paper text-green-500 mr-1"></i> Solicitudes Pendientes</h3><button onclick="window.renderizarSolicitudesAdmin()" class="text-gray-500 hover:text-white p-1"><i class="fas fa-sync-alt"></i></button></div><div id="solicitudesList" class="space-y-2"></div></div><div class="bg-black/60 p-5 rounded-2xl border border-yellow-500/20 shadow-lg relative overflow-hidden"><div class="absolute top-0 right-0 bg-yellow-500 text-black text-[8px] font-black px-3 py-1 rounded-bl-xl">MOTOR FR</div><h3 class="text-[11px] font-black text-white uppercase tracking-widest mb-4"><i class="fas fa-rocket text-yellow-500 mr-1"></i> Creador de Escalera</h3><div class="space-y-4"><div><label class="text-[9px] text-gray-400 font-bold uppercase mb-1.5 block ml-1"><i class="fas fa-dollar-sign mr-1"></i> Capital Inicial (Fondo)</label><input type="number" id="inputCapitalEscalera" value="50000" class="w-full bg-gray-900 border border-white/10 p-3.5 rounded-xl text-white font-black text-sm outline-none focus:border-yellow-500 shadow-inner"></div><div><label class="text-[9px] text-gray-400 font-bold uppercase mb-1.5 block ml-1"><i class="fas fa-crosshairs mr-1"></i> Cuota Objetivo Global</label><input type="number" id="inputCuotaObjetivo" step="0.1" value="2.0" class="w-full bg-gray-900 border border-white/10 p-3.5 rounded-xl text-white font-black text-sm outline-none focus:border-yellow-500 shadow-inner"></div><div><label class="text-[9px] text-gray-400 font-bold uppercase mb-1.5 block ml-1"><i class="fas fa-shield-alt mr-1"></i> Seguridad Mínima (%)</label><input type="number" id="inputProbMinima" value="85" class="w-full bg-gray-900 border border-white/10 p-3.5 rounded-xl text-white font-black text-sm outline-none focus:border-yellow-500 shadow-inner"></div><div><label class="text-[9px] text-gray-400 font-bold uppercase mb-1.5 block ml-1"><i class="far fa-calendar-alt mr-1"></i> Fecha del Reto</label><input type="date" id="inputFechaEscalera" class="w-full bg-gray-900 border border-white/10 p-3.5 rounded-xl text-white font-black text-xs uppercase outline-none focus:border-yellow-500 shadow-inner"></div><button onclick="window.generarRetoAdmin()" class="w-full py-4 bg-gradient-to-r from-blue-700 to-blue-600 hover:from-blue-600 hover:to-blue-500 text-white rounded-xl font-black text-[12px] uppercase tracking-widest shadow-[0_10px_20px_rgba(37,99,235,0.3)] transition active:scale-95 mt-2"><i class="fas fa-robot mr-1"></i> Analizar Mercado Global</button></div></div><div id="previewRetoAdmin" class="hidden mt-4"></div><textarea id="inputAdminReto" rows="2" class="w-full bg-black/40 border border-white/10 p-4 rounded-xl text-gray-300 text-xs mt-4 outline-none focus:border-yellow-500 hidden shadow-inner" placeholder="Escribe un mensaje de estrategia para los usuarios..."></textarea><button id="btnPublicarReto" onclick="window.publicarRetoEscalera()" class="w-full mt-4 py-4 bg-gradient-to-r from-green-600 to-green-500 text-white rounded-xl font-black text-[12px] uppercase tracking-widest shadow-[0_10px_20px_rgba(34,197,94,0.3)] transition active:scale-95 hidden"><i class="fas fa-broadcast-tower mr-1"></i> Publicar Escalera Oficial</button><button onclick="window.eliminarRetoEscaleraGlobal()" class="w-full mt-2 py-3 bg-red-600/20 hover:bg-red-600/40 text-red-400 rounded-xl border border-red-500/30 font-black text-[10px] uppercase shadow-lg transition active:scale-95"><i class="fas fa-trash-alt mr-1"></i> Borrar Reto Activo</button><div id="panelGestionRetoActivo" class="mt-4"></div><div class="bg-black/60 p-4 rounded-2xl border border-white/5 shadow-md mt-4"><div class="flex justify-between items-center mb-3"><h3 class="text-[10px] text-gray-400 font-bold uppercase"><i class="fas fa-history text-blue-500 mr-1"></i> Historial de Retos</h3><button onclick="window.cargarHistorialEscaleraAdmin()" class="text-gray-500 hover:text-white p-1"><i class="fas fa-sync-alt"></i></button></div><div id="historialEscaleraAdminList" class="space-y-3"></div></div></div><div id="vistaAdm_access" class="admin-view-content hidden space-y-4"><div class="bg-black/60 p-4 rounded-2xl border border-white/5 shadow-md"><h3 class="text-[10px] text-gray-400 font-bold uppercase mb-3"><i class="fas fa-key text-yellow-500 mr-1"></i> Generador de Licencias</h3><input type="text" id="newCodeInput" placeholder="Ej: JUANPEREZ2026" class="w-full bg-gray-900 border border-white/10 p-3.5 rounded-xl text-white text-xs mb-3 uppercase outline-none focus:border-yellow-500 shadow-inner"><div class="flex gap-2"><button onclick="window.crearCodigo(false)" id="btnCrearVip" class="flex-1 py-3 bg-yellow-600 text-black rounded-xl font-black text-[10px] uppercase shadow-lg active:scale-95 transition-transform"><i class="fas fa-star mr-1"></i> Crear VIP</button><button onclick="window.crearCodigo(true)" id="btnCrearPrem" class="flex-1 py-3 bg-purple-700 text-white rounded-xl font-black text-[10px] uppercase shadow-lg active:scale-95 transition-transform"><i class="fas fa-gem mr-1"></i> Crear PREM</button></div></div><div class="bg-black/60 p-4 rounded-2xl border border-white/5 shadow-md"><div class="flex justify-between items-center mb-3"><h3 class="text-[10px] text-gray-400 font-bold uppercase"><i class="fas fa-users text-blue-500 mr-1"></i> Base de Datos Activa</h3><button onclick="window.renderizarListaAdmin()" class="text-gray-500 hover:text-white p-1"><i class="fas fa-sync-alt"></i></button></div><div id="codesList" class="space-y-2"></div></div></div></div>`;
    window.cargarMonitorTickets(); window.cargarUsuariosAdmin(); window.cargarFondosAdmin(); window.renderizarListaAdmin(); window.renderizarSolicitudesAdmin(); window.cargarHistorialEscaleraAdmin(); window.cargarNotificacionesAdmin(); window.cargarGestionRetoActivoAdmin();
    const h = new Date(); let ms = String(h.getMonth() + 1).padStart(2, '0'); let d = String(h.getDate()).padStart(2, '0'); document.getElementById('inputFechaEscalera').value = `${h.getFullYear()}-${ms}-${d}`;
};
window.cambiarTabAdmin = function(t){ document.querySelectorAll('.admin-view-content').forEach(e=>e.classList.add('hidden')); document.querySelectorAll('.admin-tab-btn').forEach(b=>{b.classList.remove('bg-yellow-500','text-black','shadow-md');b.classList.add('text-gray-400');}); document.getElementById('vistaAdm_'+t).classList.remove('hidden'); let a=document.getElementById('tab_'+t); a.classList.remove('text-gray-400'); a.classList.add('bg-yellow-500','text-black','shadow-md'); };

window.generarRetoAdmin = async function() {
    const me = parseFloat(document.getElementById('inputCuotaObjetivo').value); const pM = parseFloat(document.getElementById('inputProbMinima').value) || 85; const fE = document.getElementById('inputFechaEscalera').value;
    if(!fE) { window.mostrarAlerta("Error", "Selecciona fecha.", "error"); return; } if(!me || me <= 1.0) { window.mostrarAlerta("Error", "Cuota > 1.0", "error"); return; }
    const b = document.querySelector('button[onclick="window.generarRetoAdmin()"]'); const oB = b.innerHTML; b.innerHTML = '<i class="fas fa-spinner animate-spin"></i> BUSCANDO...'; b.disabled = true;
    try {
        let sL = competicionesGlobales.map(l => l.key); let t = CACHE_PARTIDOS_FUTUROS.filter(p => sL.includes(p.sport_key));
        let pD = t.filter(p => { const d = new Date(p.commence_time); let m = String(d.getMonth() + 1).padStart(2, '0'); let da = String(d.getDate()).padStart(2, '0'); return `${d.getFullYear()}-${m}-${da}` === fE; });
        if(pD.length === 0) { window.mostrarAlerta("Sin Datos", "No hay eventos en caché para esa fecha.", "warning"); return; }
        let op = []; pD.forEach(p => { let v = obtenerOpcionesRentables(p); v.forEach(vo => { if(vo.probabilidad >= pM && vo.cuota >= 1.15 && vo.cuota <= 2.50) { op.push({ partido: p, ...vo }); } }); }); 
        op.sort((a,b) => { if(a.verificado_ia && !b.verificado_ia) return -1; if(!a.verificado_ia && b.verificado_ia) return 1; let sA = (a.mercadoKey.includes('spreads') || a.mercadoKey.includes('totals') || a.mercadoKey.includes('corners')) ? 2 : (a.mercadoKey === 'h2h' ? 0 : 1); let sB = (b.mercadoKey.includes('spreads') || b.mercadoKey.includes('totals') || b.mercadoKey.includes('corners')) ? 2 : (b.mercadoKey === 'h2h' ? 0 : 1); if (sA !== sB) return sB - sA; if(b.probabilidad !== a.probabilidad) return b.probabilidad - a.probabilidad; return b.cuota - a.cuota; });
        let u = []; let vi = new Set(); op.forEach(o => { if(!vi.has(o.partido.id)){ vi.add(o.partido.id); u.push(o); } });
        if(u.length === 0) { window.mostrarAlerta("Límite", `Sin apuestas válidas para un Reto (Prob >= ${pM}% y Cuota >= 1.15).`, "warning"); return; }
        let se = []; let cA = 1.0; for(let o of u) { se.push(o); cA *= o.cuota; if(cA >= me) break; if(se.length >= 2) break; }
        window.retoPendientePublicar = { picks: se.map(s => ({ home_team: s.partido.home_team, away_team: s.partido.away_team, mercadoKey: s.mercadoKey, nombre: s.nombre, point: s.point === undefined ? null : s.point, cuota: s.cuota, probabilidad: s.probabilidad, stake: 100, confirmados: [] })), cuotaTotal: cA.toFixed(2), fechaFiltro: fE };
        let pH = `<div class="bg-black/50 p-4 rounded-xl border border-yellow-500/50 shadow-[0_0_15px_rgba(212,175,55,0.2)]"><div class="flex justify-between items-center mb-3 border-b border-white/10 pb-2"><span class="text-xs font-black text-yellow-500 uppercase"><i class="fas fa-ticket-alt mr-1"></i> TICKET (PREVIEW)</span><span class="text-[10px] bg-yellow-500 text-black font-black px-2 py-0.5 rounded">Cuota: ${cA.toFixed(2)}</span></div>`;
        se.forEach((p, i) => { let dM = definicionesApuestas[p.mercadoKey] || {titulo: 'Mercado Especial'}; let pT = formatearPickEspanol(p.nombre, p.point, p.mercadoKey); let ic = "fa-handshake"; if(p.mercadoKey.includes('shots')) ic = "fa-bullseye"; else if(p.mercadoKey.includes('corners')) ic = "fa-flag"; else if(p.mercadoKey.includes('cards')) ic = "fa-square"; else if(p.mercadoKey === 'totals') ic = "fa-futbol"; else if(p.mercadoKey === 'spreads') ic = "fa-balance-scale"; pH += `<div class="bg-gray-900/50 p-3 rounded-lg mb-3 border border-white/5 relative"><div class="absolute top-0 right-0 bg-green-600 text-white text-[8px] font-black px-2 py-1 rounded-bl-lg shadow-md">CONF: ${p.probabilidad}%</div><div class="text-[8px] text-gray-400 font-bold uppercase mb-1"><i class="fas ${ic} mr-1"></i> ${dM.titulo || dM}</div><div class="text-[11px] font-bold text-white mb-2 border-b border-white/5 pb-1">${p.partido.home_team} <span class="text-gray-500 font-normal mx-1">vs</span> ${p.partido.away_team}</div><div class="flex justify-between items-center bg-black/60 p-2 rounded border border-gray-700"><span class="text-[10px] text-yellow-500 font-black uppercase tracking-wide">PICK: ${pT}</span><span class="text-white font-black text-xs">${parseFloat(p.cuota).toFixed(2)}</span></div><div class="mt-2 pt-2 border-t border-white/10 flex justify-between items-center"><span class="text-[9px] text-blue-400 uppercase font-bold"><i class="fas fa-percentage mr-1"></i> % de Fondo a Apostar</span><input type="number" value="100" class="w-20 bg-blue-900/30 text-blue-400 font-black text-center text-xs p-1.5 rounded border border-blue-500/50 outline-none" onchange="window.actualizarStakePreview(${i}, this.value)"></div></div>`; });
        pH += `</div>`; document.getElementById('previewRetoAdmin').innerHTML = pH; document.getElementById('previewRetoAdmin').classList.remove('hidden'); document.getElementById('inputAdminReto').value = `⚠️ Gestión de Banca Sugerida: Respeta el % indicado en cada pick.`; document.getElementById('inputAdminReto').classList.remove('hidden'); document.getElementById('btnPublicarReto').classList.remove('hidden');
        if (cA < me) window.mostrarAlerta("Aviso", `Se logró cuota ${cA.toFixed(2)}.`, "warning"); else window.mostrarAlerta("Generado", `Cuota: ${cA.toFixed(2)}. Revisa el % de Stake antes de publicar.`, "success");
    } catch(e) { window.mostrarAlerta("Error", "Fallo IA.", "error"); } finally { b.innerHTML = oB; b.disabled = false; }
};

window.actualizarStakePreview = function(i, v) { let va = parseFloat(v); if(!isNaN(va) && va > 0 && va <= 100 && window.retoPendientePublicar) { window.retoPendientePublicar.picks[i].stake = va; } };

window.publicarRetoEscalera = async function() {
    const t = document.getElementById('inputAdminReto').value; if(!t && !window.retoPendientePublicar) return window.mostrarAlerta("Error", "Nada para publicar.", "error");
    const b = document.getElementById('btnPublicarReto'); const oT = b.innerText; b.innerText = "Publicando..."; b.disabled = true;
    const cS = document.getElementById('inputCapitalEscalera')?.value; const cI = parseFloat(cS) || 50000;
    window.retoPendientePublicar.capital_inicial = cI; window.retoPendientePublicar.estado_reto = 'activo';
    ...
    try { 
        const a = Date.now(); await setDoc(doc(db, "global", "escalera"), { mensaje: t, ticket_data: window.retoPendientePublicar, timestamp: a }); 
        const iH = a.toString(); const fS = new Date().toLocaleDateString('es-CO', {timeZone: 'America/Bogota'}) + ' ' + new Date().toLocaleTimeString('es-CO', {timeZone: 'America/Bogota', hour: '2-digit', minute:'2-digit'});
        await setDoc(doc(db, "historial_escalera", iH), { id: iH, mensaje: t, ticket_data: window.retoPendientePublicar, fecha: fS, timestamp: a });
        await setDoc(doc(collection(db, "notificaciones_push")), { titulo: "🔥 NUEVO RETO ESCALERA", cuerpo: "El algoritmo ha publicado el ticket oficial.", url: window.location.origin + "/?view=escalera", timestamp: a, enviadoPor: "FR (Bot)", audiencia: "escalera" });
    ...
    } catch(e){ window.mostrarAlerta("Error", "Error de red.", "error"); } finally { b.innerText = oT; b.disabled = false; }
};

window.cargarGestionRetoActivoAdmin = async function() {
    const p = document.getElementById('panelGestionRetoActivo'); if(!p) return;
    try {
        const s = await getDoc(doc(db, "global", "escalera"));
        if(s.exists()) {
            const d = s.data(); const tk = d.ticket_data;
            if(!tk || !tk.picks || tk.picks.length === 0) { p.innerHTML = ''; return; }
            let cI = tk.capital_inicial || 50000; let cC = cI; let hP = '';
            
            tk.picks.forEach((pi, i) => {
                let sP = pi.stake !== undefined ? parseFloat(pi.stake) : 100; let sF = sP / 100;
                let aTB = cC * sF; let aK = cC - aTB; let wC = aK + (aTB * pi.cuota); let lC = aK;
                let sPG = (cC / cI) * 100; let ePG = (wC / cI) * 100;
                if (pi.estado === 'won') { cC = wC; } else if (pi.estado === 'lost') { cC = lC; ePG = (lC / cI) * 100; }
                let bS = pi.estado === 'won' ? 'bg-green-900/30 border-green-500/50' : (pi.estado === 'lost' ? 'bg-red-900/30 border-red-500/50' : 'bg-gray-900/50 border-white/10');
                let cP = pi.estado === 'won' ? 'text-green-400' : (pi.estado === 'lost' ? 'text-red-400' : 'text-gray-400');
                let cc = pi.confirmados ? pi.confirmados.length : 0; let cl = cc > 0 ? pi.confirmados.join(', ') : 'Nadie aún';
                hP += `<div class="${bS} p-3 rounded-lg mb-2 border flex flex-col relative"><div class="flex justify-between items-center mb-2 border-b border-white/5 pb-2"><div class="flex flex-col w-1/2 pr-2"><span class="text-[9px] font-bold text-white truncate">${pi.home_team} vs ${pi.away_team}</span><span class="text-[8px] text-yellow-500 truncate">PICK: ${pi.nombre} (C: ${pi.cuota})</span></div><div class="flex gap-1 items-center justify-end w-1/2"><button onclick="window.editarStakePickEscalera(${i}, ${sP})" class="bg-blue-600/20 text-blue-400 p-2 rounded-lg border border-blue-500/30 hover:bg-blue-600/40 active:scale-95 transition flex items-center gap-1 font-black text-[9px]"><i class="fas fa-percent"></i> ${sP}%</button><div class="w-[1px] h-6 bg-gray-700 mx-1"></div><button onclick="window.marcarPickEscalera(${i}, 'won')" class="bg-green-600/20 text-green-500 p-2 rounded-lg border border-green-500/30 hover:bg-green-600/40 active:scale-95 transition"><i class="fas fa-check"></i></button><button onclick="window.marcarPickEscalera(${i}, 'lost')" class="bg-red-600/20 text-red-500 p-2 rounded-lg border border-red-500/30 hover:bg-red-600/40 active:scale-95 transition"><i class="fas fa-times"></i></button><button onclick="window.marcarPickEscalera(${i}, 'pendiente')" class="bg-gray-600/20 text-gray-400 p-2 rounded-lg border border-gray-500/30 hover:bg-gray-600/40 active:scale-95 transition"><i class="fas fa-undo"></i></button><button onclick="window.eliminarPickEscalera(${i})" class="text-red-500 hover:text-red-400 p-2 ml-1 transition"><i class="fas fa-trash-alt"></i></button></div></div><div class="flex justify-between items-center pt-1"><span class="text-[8px] text-gray-500 uppercase font-bold">Fondo Acum.</span><span class="text-[10px] font-black ${cP}">${sPG.toFixed(1)}% <i class="fas fa-arrow-right text-[8px] mx-1"></i> ${ePG.toFixed(1)}%</span></div><div class="mt-2 pt-2 border-t border-white/5"><span class="text-[8px] text-blue-400 font-bold uppercase"><i class="fas fa-users mr-1"></i> Confirmados (${cc}):</span><div class="text-[7px] text-gray-400 mt-1 break-words">${cl}</div></div></div>`;
            });
            let tPR = (cC / cI) * 100;
            let h = `<div class="bg-black/60 p-4 rounded-2xl border border-yellow-500/50 shadow-lg relative"><div class="absolute top-0 right-0 bg-yellow-500 text-black text-[8px] font-black px-3 py-1 rounded-bl-xl">CONTROL LIVE</div><h3 class="text-[11px] font-black text-white uppercase tracking-widest mb-3"><i class="fas fa-gamepad text-yellow-500 mr-1"></i> Tablero</h3><div class="grid grid-cols-3 gap-2 mb-4"><div class="bg-gray-900 border border-white/5 p-2 rounded-lg text-center"><span class="block text-[7px] text-gray-500 uppercase">C. Inicial</span><span class="text-gray-400 font-black text-xs">${formatoCOP(cI)}</span></div><div class="bg-black border ${tk.estado_reto==='perdido'?'border-red-500/30':'border-yellow-500/50'} p-2 rounded-lg text-center"><span class="block text-[7px] ${tk.estado_reto==='perdido'?'text-red-500':'text-yellow-500'} uppercase">Actual</span><span class="${tk.estado_reto==='perdido'?'text-red-500':'text-yellow-500'} font-black text-sm">${formatoCOP(cC)}</span></div><div class="bg-blue-900/20 border border-blue-500/30 p-2 rounded-lg text-center"><span class="block text-[7px] text-blue-400 uppercase">Crecimiento</span><span class="text-blue-500 font-black text-sm">${tPR.toFixed(1)}%</span></div></div>${hP}</div>`;
            p.innerHTML = h;
        } else { p.innerHTML = ''; }
    } catch(e) {}
};
...
window.editarStakePickEscalera = async function(i, cS) {
    let n = prompt("Ingresa el % de inversión (ej: 100, 50, 25):", cS); let v = parseFloat(n);
    if(!isNaN(v) && v > 0 && v <= 100) {
        try {
            const s = await getDoc(doc(db, "global", "escalera"));
            if(s.exists()) {
                let d = s.data(); d.ticket_data.picks[i].stake = v;
...
window.eliminarPickEscalera = async function(i) {
    window.mostrarConfirmacion("Borrar Pick", "¿Eliminar esta selección?", async () => {
        try {
            const s = await getDoc(doc(db, "global", "escalera"));
            if(s.exists()) {
                const d = s.data(); d.ticket_data.picks.splice(i, 1);
                let p = 0; let g = 0; d.ticket_data.picks.forEach(pi => { if(pi.estado === 'lost') p++; if(pi.estado === 'won') g++; });
                if(d.ticket_data.picks.length === 0) d.ticket_data.estado_reto = 'activo'; else if(p > 0) d.ticket_data.estado_reto = 'perdido'; else if(g === d.ticket_data.picks.length) d.ticket_data.estado_reto = 'ganado'; else d.ticket_data.estado_reto = 'activo';
                await updateDoc(doc(db, "global", "escalera"), { ticket_data: d.ticket_data }); window.cargarGestionRetoActivoAdmin();
                if(document.getElementById('vista_escalera').classList.contains('view-active')) window.cargarRetoEscaleraNube();
...
window.marcarPickEscalera = async function(i, e) {
    try {
        const s = await getDoc(doc(db, "global", "escalera"));
        if(s.exists()) {
            const d = s.data(); d.ticket_data.picks[i].estado = e;
            let p = 0; let g = 0; d.ticket_data.picks.forEach(pi => { if(pi.estado === 'lost') p++; if(pi.estado === 'won') g++; });
            if(p > 0) d.ticket_data.estado_reto = 'perdido'; else if(g === d.ticket_data.picks.length) d.ticket_data.estado_reto = 'ganado'; else d.ticket_data.estado_reto = 'activo';
            await updateDoc(doc(db, "global", "escalera"), { ticket_data: d.ticket_data }); window.cargarGestionRetoActivoAdmin();
            if(document.getElementById('vista_escalera').classList.contains('view-active')) window.cargarRetoEscaleraNube();
        }
    } catch(ex) { window.mostrarAlerta("Error", "No se pudo actualizar", "error"); }
};
...
window.cargarHistorialEscaleraAdmin = async function() {
    const l = document.getElementById('historialEscaleraAdminList'); if(!l) return; l.innerHTML = '<div class="text-center p-4"><i class="fas fa-spinner animate-spin text-blue-500"></i></div>';
    try {
        const q = query(collection(db, "historial_escalera"), orderBy("timestamp", "desc"), limit(20)); const s = await getDocs(q); l.innerHTML = '';
        if(s.empty) { l.innerHTML = `<p class="text-[10px] text-gray-500 text-center border border-dashed border-white/10 p-4">No hay retos.</p>`; return; }
        s.forEach(doc => {
...
window.crearCodigo = async function(eI) { const iC = document.getElementById('newCodeInput'); if(!iC) return; const nC = iC.value.toUpperCase().trim(); if(!nC || nC === 'UNDEFINED' || nC === 'NULL') { return window.mostrarAlerta("Error", "Código inválido.", "error"); } const r = doc(db, "codigos_nube", nC); try { const dS = await getDoc(r); if(dS.exists()) { return window.mostrarAlerta("Error", "Ya existe.", "error"); } const a = Date.now(); await setDoc(r, { code: nC, ilimitado: eI, deviceID: null, ladderStatus: 'none', creado: a }); iC.value = ''; window.mostrarAlerta("Éxito", `Código creado.`, "success"); if(window.renderizarListaAdmin) window.renderizarListaAdmin(); } catch (error) { window.mostrarAlerta("Error", "Fallo de conexión al servidor.", "error"); } };
...
window.resetearDispositivo = async function(c) { window.mostrarConfirmacion("Resetear", `¿Permitir nuevo login para ${c}?`, async () => { try { await updateDoc(doc(db, "codigos_nube", c), { deviceID: null }); window.renderizarListaAdmin(); window.mostrarAlerta("Liberado", "Login liberado.", "success"); } catch(e){} }); };
...
window.renderizarListaAdmin = async function() {
    ...
    try { const s = await getDocs(collection(db, "codigos_nube")); l.innerHTML = ''; let a = []; s.forEach(d => a.push(d.data())); a.sort((x, y) => y.creado - x.creado); a.forEach(c => { let co = c.ilimitado ? 'text-purple-400 bg-purple-900/20 border-purple-500/30' : 'text-yellow-500 bg-yellow-900/20 border-yellow-500/30'; let st = c.deviceID ? '<span class="text-red-400"><i class="fas fa-lock"></i> Usado</span>' : '<span class="text-green-400"><i class="fas fa-unlock"></i> Libre</span>'; let bR = c.deviceID ? `<button onclick="window.resetearDispositivo('${c.code}')" class="text-blue-400 hover:text-blue-300 transition-colors p-2 bg-blue-900/20 border border-blue-500/30 rounded-lg mr-2"><i class="fas fa-unlock-alt"></i></button>` : ''; l.innerHTML += `<div class="flex justify-between items-center bg-black/40 p-3 rounded-lg border border-white/5 mb-2 shadow-sm"><div class="flex flex-col"><span class="text-white font-black text-xs tracking-wider">${c.code}</span><div class="flex gap-2 mt-1 text-[8px] uppercase font-bold"><span class="px-1.5 py-0.5 rounded border ${co}">${c.ilimitado ? 'Premium' : 'VIP'}</span>${st}</div></div><div class="flex items-center">${bR}<button onclick="window.eliminarCodigo('${c.code}')" class="text-gray-500 hover:text-red-500 transition-colors p-2 bg-white/5 rounded-lg"><i class="fas fa-trash"></i></button></div></div>`; }); } catch (e) { l.innerHTML = `<p class="text-red-500 text-xs">Error.</p>`; }
};

window.renderizarSolicitudesAdmin = async function() { const l = document.getElementById('solicitudesList'); if(!l) return; l.innerHTML = '<div class="text-center p-4"><i class="fas fa-spinner animate-spin text-green-500"></i></div>'; try { const s = await getDocs(collection(db, "codigos_nube")); l.innerHTML = ''; let c = 0; s.forEach(d => { let da = d.data(); if(da.ladderStatus === 'pending') { c++; l.innerHTML += `<div class="flex justify-between items-center bg-black/60 p-3 rounded-lg border border-green-500/30 mb-2 shadow-md"><span class="text-white font-black text-[11px] uppercase"><i class="fas fa-user text-green-500 mr-2"></i>${da.code}</span><button onclick="window.aprobarEscalera('${da.code}')" class="bg-green-600 hover:bg-green-500 text-white px-4 py-2 rounded-md text-[9px] font-black uppercase transition-all active:scale-95 shadow-lg">Aprobar</button></div>`; } }); if(c === 0) { l.innerHTML = `<p class="text-[10px] text-gray-500 text-center py-4">No hay solicitudes</p>`; } } catch(e){} };
...
window.cargarMonitorTickets = async function() {
    ...
    try {
        ...
        if(s.empty) { l.innerHTML = `<p class="text-[10px] text-gray-500 text-center border border-dashed border-white/10 p-4">Nadie ha guardado tickets.</p>`; return; }
        s.forEach(doc => {
            let t = doc.data(); let pH = ''; let aP = t.picksObj || t.picks || [];
            if(aP.length > 0 && typeof aP[0] === 'object') { pH = aP.map(p => { let m = p.mercadoKey || p.mercado || ''; const dM = definicionesApuestas[m] || { 'titulo': 'Mercado Especial' }; let ic = "fa-handshake"; if(m.includes('shots')) ic = "fa-bullseye"; else if(m.includes('corners')) ic = "fa-flag"; let bIA = p.verificado_ia ? `<span class="text-blue-400 text-[8px] ml-1"><i class="fas fa-gem"></i></span>` : ''; return `<div class="bg-gray-900/80 p-2 rounded mb-1 border border-white/5 relative"><div class="text-[7px] text-gray-400 font-bold uppercase mb-0.5"><i class="fas ${ic} mr-1"></i> ${dM.titulo}</div><div class="text-[9px] font-bold text-white leading-tight mb-1">${p.partido}</div><div class="flex justify-between items-center"><span class="text-[8px] text-yellow-500 font-black uppercase">PICK: ${p.pick} ${bIA}</span><span class="text-white font-black text-[10px]">${p.cuota ? parseFloat(p.cuota).toFixed(2) : ''}</span></div></div>`; }).join(''); } else if (aP.length > 0) { pH = aP.map(p => `<li class="text-[8px] text-gray-300 border-b border-white/5 py-1 last:border-0">${p}</li>`).join(''); }
            l.innerHTML += `<div class="bg-black/60 p-3 rounded-xl border border-yellow-500/20 relative shadow-md mb-3"><div class="absolute top-0 right-0 flex overflow-hidden rounded-bl-xl rounded-tr-xl shadow-md z-10"><button onclick="window.eliminarTicketAdmin('${doc.id}', 'dash')" class="bg-red-600 hover:bg-red-500 text-white text-[10px] px-3 py-1 transition-colors"><i class="fas fa-trash"></i></button><span class="bg-yellow-500 text-black text-[10px] font-black px-3 py-1">C: ${t.cuota}</span></div><div class="flex justify-between items-center mb-2 border-b border-white/5 pb-1 mt-1 pr-14"><span class="text-[11px] font-black text-white uppercase"><i class="fas fa-user-secret text-yellow-500 mr-1"></i> ${t.codigo_usuario}</span><span class="text-[8px] text-gray-500">${t.fecha}</span></div><div class="mt-2">${pH}</div></div>`;
        });
    } catch(e) {}
};

let ticketsGlobalesAdminCache = []; window.usuariosAdminList = [];
window.cargarUsuariosAdmin = async function(fR = false) { const c = document.getElementById('listaUsuariosAdminContainer'); if(!c) return; if(!fR && ticketsGlobalesAdminCache.length > 0) { window.renderizarUsuariosFiltrados(); return; } c.innerHTML = '<div class="text-center p-4"><i class="fas fa-spinner animate-spin text-yellow-500"></i></div>'; try { const q = query(collection(db, "tickets_guardados"), orderBy("timestamp", "desc")); const s = await getDocs(q); ticketsGlobalesAdminCache = []; let mU = {}; s.forEach(doc => { let t = doc.data(); t.id = doc.id; ticketsGlobalesAdminCache.push(t); let u = t.codigo_usuario || 'Desconocido'; if(!mU[u]) mU[u] = { uid: u, count: 0, last: t.timestamp }; mU[u].count++; if(t.timestamp > mU[u].last) mU[u].last = t.timestamp; }); window.usuariosAdminList = Object.values(mU).sort((a, b) => b.last - a.last); window.renderizarUsuariosFiltrados(); if(document.getElementById('panelDetalleUsuarioAdmin') && !document.getElementById('panelDetalleUsuarioAdmin').classList.contains('hidden')) { let ti = document.getElementById('tituloDetalleUsuarioAdmin').innerText; let cU = ti.replace('TICKETS DE: ', ''); window.verDetalleUsuarioAdmin(cU); } } catch(e) {} };
window.renderizarUsuariosFiltrados = function(f = '') { const c = document.getElementById('listaUsuariosAdminContainer'); let fi = window.usuariosAdminList.filter(u => u.uid.toLowerCase().includes(f.toLowerCase())); if(fi.length === 0) { c.innerHTML = '<p class="text-gray-500 text-[10px] text-center py-4">No se encontraron usuarios.</p>'; return; } let h = ''; fi.forEach(u => { h += `<button onclick="window.verDetalleUsuarioAdmin('${u.uid}')" class="w-full bg-black/40 hover:bg-black/60 border border-white/5 p-4 rounded-xl flex justify-between items-center transition-all shadow-sm active:scale-95 mb-2"><div class="flex items-center gap-3"><div class="bg-gray-800 p-3 rounded-full text-yellow-500 shadow-inner"><i class="fas fa-user-astronaut text-lg"></i></div><div class="flex flex-col items-start"><span class="text-white font-black text-xs uppercase tracking-wider">${u.uid}</span><span class="text-gray-400 text-[9px] font-bold mt-0.5 px-2 bg-gray-800 rounded">${u.count} Tickets Guardados</span></div></div><i class="fas fa-chevron-right text-gray-600"></i></button>`; }); c.innerHTML = h; };
...
window.cargarFondosAdmin = async function() {
    const c = document.getElementById('listaFondoAdminContainer'); if(!c) return; c.innerHTML = '<div class="text-center p-4"><i class="fas fa-spinner animate-spin text-green-500"></i></div>';
    try {
