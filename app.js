...
// ==========================================
// 3. FUNCIONES VITALES Y NOTIFICACIONES
// ==========================================
...
window.registrarTokenPush = async function(codigoUsuario, modoSilencioso = false) {
  ...
  try {
    ...
    if (permission === 'granted') {
      ...
      if(!modoSilencioso) window.mostrarAlerta("Permiso Denegado", "Has bloqueado las alertas.", "warning");
...
window.iniciarSwipeNotificaciones = function() {
  ...
  cards.forEach(card => {
    ...
    card.addEventListener('touchstart', e => {
      startX = e.touches[0].clientX; isDragging = true; window.isSwiping = false; card.style.transition = 'none';
    }, {passive: true});
...
window.abrirBandejaNotificaciones = async function() {
  if (!modoVipActivo && !adminAutenticado) {
    window.mostrarAlerta("Acceso Restringido", "Debes iniciar sesión con tu credencial para leer los comunicados.", "warning");
    window.abrirModalLogin();
    return;
  }
  ...
  try {
    ...
    lista.innerHTML = `<div class="text-center text-red-500 text-xs">Error de red.</div>`; 
  }
};

window.cerrarBandejaNotificaciones = function() { 
  const modal = document.getElementById('modalBandejaNotificaciones'); 
  const contenido = document.getElementById('bandejaContenido');
    ...
    if (cacheGuardado && tiempoGuardado && (ahoraMs - parseInt(tiempoGuardado)) < 900000) {
      ...
      CACHE_PARTIDOS_FUTUROS.forEach(p => {
        if(!ligasMap[p.sport_key]) { 
          ligasMap[p.sport_key] = { key: p.sport_key, title: p.sport_title, group: p.sport_group || 'Soccer' }; 
        }
      });
...
// 🚀 REPARACIÓN 1: CARGAR LA BASE DE DATOS ANTES DE CUALQUIER OTRA COSA
window.iniciarApp = async function() { 
  ...
  // OBLIGATORIO: Se descargan los eventos antes de evaluar si es admin
...
// ==========================================
// 5. MÉTODOS DE UI Y ALERTAS GLOBALES
// ==========================================
...
window.cerrarModalLogin = function() { 
  ...
  if(inputElement) { 
    ...
    inputElement.placeholder = 'CÓDIGO DE ACCESO'; 
    inputElement.value = ''; 
  } 
...
window.mostrarAlerta = function(titulo, mensaje, tipo = 'info', actionUrl = null) {
  ...
  if(actionUrl) { btn.innerHTML = '<i class="fas fa-rocket mr-1"></i> IR A LA SEÑAL'; } 
  else { btn.innerHTML = 'ACEPTAR'; }
...
// ==========================================
// 6. MOTOR DE LOGIN Y SEGURIDAD
// ==========================================
const promesaConTimeout = (promesa, ms) => { 
  let timeout = new Promise((resolve, reject) => { 
    let id = setTimeout(() => { clearTimeout(id); reject(new Error("Timeout")); }, ms); 
  }); 
...
// 🚀 REPARACIÓN 2: LIMPIEZA DE ESPACIOS EN EL CORREO DEL ADMIN
  ...
  if (codigoIngresado.startsWith("MASTER_")) {
    try {
      if (!correoAdminTemp) {
        ...
        correoAdminTemp = pmt.trim(); // ESTO DESTRUYE EL ESPACIO EN BLANCO DEL TECLADO
      }
...
window.concederAcceso = function(esIlimitado, codeString, ladderStat, esModoBackground = false) {
  ...
  try { 
    localStorage.setItem('oracle_session', JSON.stringify({ code: codeString, ilimitado: esIlimitado, ladderStat: ladderStat })); 
  } catch(e) {}
  ...
  if(btnTop) {
    ...
    if(esIlimitado) { nuevoBtn.className = "shrink-0 bg-purple-900 text-purple-300 text-[9px] px-3 py-2 rounded-full font-black uppercase border border-purple-500/50 flex items-center gap-1.5 transition-all"; } 
    else { nuevoBtn.className = "shrink-0 bg-yellow-900 text-yellow-500 text-[9px] px-3 py-2 rounded-full font-black uppercase border border-yellow-600/50 flex items-center gap-1.5 transition-all"; }
  }
...
// ==========================================
// 7. RUTEO Y RENDERIZADO VISUAL DE PARTIDOS
// ==========================================
window.cambiarVista = function(vista) {
  ...
  ['picks', 'historial', 'escalera', 'apadrinamiento'].forEach(b => { 
    ...
    if(btn) { 
      if(b === vista) { btn.classList.remove('text-gray-500'); btn.classList.add('text-yellow-500'); } 
      else { btn.classList.add('text-gray-500'); btn.classList.remove('text-yellow-500'); } 
    } 
  });
...
window.construirMenuLateral = function() {
  ...
  competicionesGlobales.forEach(liga => { 
    ...
    arbol[deporte][info.pais].push({ key: liga.key, name: info.nombreLiga, bandera: info.bandera }); 
  });
  ...
  Object.keys(arbol).sort().forEach(deporte => { 
    ...
    if(deporte === 'Baloncesto') iconDep = 'fa-basketball-ball'; else if(deporte === 'Tenis') iconDep = 'fa-table-tennis'; else if(deporte === 'Fútbol Americano' || deporte === 'American Football') iconDep = 'fa-football-ball'; else if(deporte === 'Béisbol' || deporte === 'Baseball') iconDep = 'fa-baseball-ball'; else if(deporte === 'Hockey') iconDep = 'fa-hockey-puck'; else if(deporte === 'MMA' || deporte === 'UFC') iconDep = 'fa-hand-rock'; else if(deporte === 'Boxeo') iconDep = 'fa-mitten'; 
...
window.aplicarFiltrosLocales = function() {
  ...
  if(modoVipActivo) { if(window.renderizarPartidosVIP) window.renderizarPartidosVIP(); } 
  else { if(window.renderizarPartidosFree) window.renderizarPartidosFree(); }
};
...
function obtenerOpcionesRentables(partido) {
  let mapaResultados = {}; 
  if (!partido.bookmakers) return [];
  
  partido.bookmakers.forEach(b => { 
    b.markets?.forEach(m => { 
      m.outcomes?.forEach(o => { 
        let nombreOpcion = o.name || ""; 
        let descripcion = o.description || ""; 
        if (descripcion && descripcion !== nombreOpcion) { 
          nombreOpcion = `${descripcion} | ${nombreOpcion}`; 
        } 
        let k = `${m.key}-${nombreOpcion}-${o.point||''}`; 
        if(!mapaResultados[k]) { 
          mapaResultados[k] = { mercadoKey: m.key, nombre: nombreOpcion, point: o.point, cuotas: [], brokers: [] }; 
        } 
        if(o.price > 1.0) { 
          mapaResultados[k].cuotas.push(o.price); 
          mapaResultados[k].brokers.push({ 
            broker: b.title, cuota: o.price, probabilidad_real: o.probabilidad_real || null, 
            ev_porcentaje: o.ev_porcentaje || null, es_valor: o.es_valor || false, verificado_ia: o.verificado_ia || false 
          }); 
        } 
      }); 
    }); 
  ...
  Object.values(mapaResultados).forEach(res => { 
    ...
    if(res.cuotas.length >= liquidezMinima) { 
      res.brokers.sort((a,b) => b.cuota - a.cuota); 
      let mejorOpcion = res.brokers[0]; 
      let probFinal = mejorOpcion.probabilidad_real; 
      let edgeFinal = mejorOpcion.ev_porcentaje; 
      
      if (probFinal === null || probFinal === undefined) { 
        let cuotaPromedio = res.cuotas.reduce((a,b) => a+b, 0) / res.cuotas.length; 
        probFinal = Math.min(Math.round((1 / cuotaPromedio) * 100), 99); 
        edgeFinal = parseFloat((((mejorOpcion.cuota / cuotaPromedio) - 1) * 100).toFixed(2)); 
      } 
      if(mejorOpcion.cuota >= 1.05 && mejorOpcion.cuota <= 4.50) { 
        opcionesFinales.push({ 
          broker: mejorOpcion.broker, mercadoKey: res.mercadoKey, nombre: res.nombre, point: res.point, 
          cuota: mejorOpcion.cuota, probabilidad: probFinal, edgeValor: edgeFinal, 
          es_valor: mejorOpcion.es_valor || (edgeFinal > 0), verificado_ia: mejorOpcion.verificado_ia 
        }); 
      } 
    } 
  });
  ...
  opcionesFinales.sort((a, b) => { 
    ...
    if (!a.verificado_ia && b.verificado_ia) return 1; 
    if (a.es_valor && !b.es_valor) return -1; 
    if (!a.es_valor && b.es_valor) return 1; 
    if (b.edgeValor !== a.edgeValor) return b.edgeValor - a.edgeValor; 
...
window.procesarTicketVIP = function() {
  if(seleccionesVIPGlobal.length < 1) return; 
  const resDiv = document.getElementById('resultadoVIP'); 
  if(!resDiv) return;
  
  resDiv.innerHTML = `<div class="p-10 text-center bg-black/40 rounded-xl border border-white/5"><i class="fas fa-satellite-dish animate-pulse text-yellow-500 mb-4 text-4xl"></i><p class="text-[11px] uppercase text-yellow-500 font-black">Analizando probabilidades...</p></div>`; 
  window.scrollTo({ top: document.body.scrollHeight, behavior: 'smooth' });
  
  setTimeout(() => { 
    ticketDinamicoVIP = []; 
    modoMercadoGlobal = 'mixto'; 
    modoRiesgoGlobal = false; 
    
    seleccionesVIPGlobal.forEach(p => { 
      let oFinales = obtenerOpcionesRentables(p); 
      if(oFinales.length > 0) { 
        let riskIdx = oFinales.findIndex(opt => opt.cuota >= 2.0 && opt.cuota <= 3.8); 
        if(riskIdx === -1) riskIdx = oFinales.length - 1; 
        ticketDinamicoVIP.push({ partido: p, opciones: oFinales, indexSeleccionado: 0, indexRiesgo: riskIdx }); 
      } 
    }); 
    window.dibujarTicketDinamico(false); 
  }, 1000);
...
window.mejorarProbabilidadTicket = function() {
  ...
  function fuzzyMatchLocal(s1, s2) { 
    let t1 = s1.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9]/g, ""); 
    let t2 = s2.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9]/g, ""); 
    if(t1.length < 3 || t2.length < 3) return t1 === t2; 
  ...
  ticketDinamicoVIP.forEach(item => {
    ...
    const oOriginal = item.opciones[idxUsado]; 
    let candidatos = []; 
    let origLower = oOriginal.nombre.toLowerCase();
    ...
    p.bookmakers?.forEach(b => { 
      b.markets?.forEach(m => { 
        m.outcomes?.forEach(o => { 
          let nombreOpcion = o.name || ""; 
          let descripcion = o.description || ""; 
          let nombreReal = descripcion && descripcion !== nombreOpcion ? `${descripcion} | ${nombreOpcion}` : nombreOpcion; 
          let nombreLower = nombreReal.toLowerCase(); 
          let esCandidato = false; 
          let pointNum = parseFloat(o.point); 
          let esMismoEquipo = fuzzyMatchLocal(nombreReal, oOriginal.nombre); 
          
          if (oOriginal.mercadoKey === 'h2h' && m.key === 'spreads' && !origLower.includes('draw') && !origLower.includes('empate') && esMismoEquipo) { 
            if (!isNaN(pointNum) && pointNum >= 0) esCandidato = true; 
          } else if (oOriginal.mercadoKey.includes('totals') && m.key === oOriginal.mercadoKey) { 
            if ((nombreLower.includes('over') || origLower.includes('over') || nombreLower.includes('más') || origLower.includes('más')) && pointNum < parseFloat(oOriginal.point)) { esCandidato = true; } 
          } else if (oOriginal.mercadoKey.includes('totals') && m.key === oOriginal.mercadoKey) { 
            if ((nombreLower.includes('under') || origLower.includes('under') || nombreLower.includes('menos') || origLower.includes('menos')) && pointNum > parseFloat(oOriginal.point)) { esCandidato = true; } 
          } else if (oOriginal.mercadoKey === 'spreads' && m.key === 'spreads' && esMismoEquipo) { 
            if (!isNaN(pointNum) && pointNum > parseFloat(oOriginal.point)) esCandidato = true; 
          } 
          
          if (esCandidato && o.price >= 1.05 && o.price < oOriginal.cuota) { 
            let probAprox = Math.min(98, Math.round((1 / o.price) * 105)); 
            let edgeMejora = oOriginal.edgeValor > 0 ? parseFloat((oOriginal.edgeValor * 0.5).toFixed(2)) : 0; 
            candidatos.push({ broker: b.title, mercadoKey: m.key, nombre: nombreReal, point: o.point, cuota: o.price, probabilidad: probAprox, edgeValor: edgeMejora, es_valor: edgeMejora > 0, verificado_ia: oOriginal.verificado_ia, es_mejora: true }); 
          } 
        }); 
      }); 
    });
    ...
    if (candidatos.length > 0) { 
      ...
      let mejorCandidato = candidatos.find(c => c.cuota >= 1.15) || candidatos[0]; 
      ...
      if (existeIdx !== -1) { 
        if (!modoRiesgoGlobal) item.indexSeleccionado = existeIdx; else item.indexRiesgo = existeIdx; 
      } else { 
        item.opciones.push(mejorCandidato); 
        if (!modoRiesgoGlobal) item.indexSeleccionado = item.opciones.length - 1; else item.indexRiesgo = item.opciones.length - 1; 
      } 
      huboMejoras = true; 
    }
  });
  
  if (huboMejoras) { window.mostrarAlerta("Ticket Blindado", "Se han encontrado líneas de protección bajando drásticamente el riesgo.", "success"); window.dibujarTicketDinamico(false); } 
  else { window.mostrarAlerta("Límites Alcanzados", "El mercado no ofrece Hándicaps Asiáticos o líneas más seguras para estos equipos en este momento.", "warning"); }
};
...
window.cambiarModoMercado = function(modo) { 
  ...
  ticketDinamicoVIP.forEach(item => { 
    ...
    if(modo === 'props') { 
      const idxS = item.opciones.findIndex(o => isProp(o.mercadoKey)); 
      if (idxS !== -1) { 
        item.indexSeleccionado = idxS; let lastIdxR = -1; 
        item.opciones.forEach((o, i) => { if (isProp(o.mercadoKey)) lastIdxR = i; }); 
        item.indexRiesgo = lastIdxR !== -1 ? lastIdxR : idxS; encontroOpciones = true; 
      } 
    } else if(modo === 'h2h' || modo === 'totals' || modo === 'spreads') { 
      const idxS = item.opciones.findIndex(o => o.mercadoKey === modo); 
      if(idxS !== -1) { 
        item.indexSeleccionado = idxS; let lastIdxR = -1; 
        item.opciones.forEach((o, i) => { if(o.mercadoKey === modo) lastIdxR = i; }); 
        item.indexRiesgo = lastIdxR !== -1 ? lastIdxR : idxS; encontroOpciones = true; 
      } 
    } else { 
      item.indexSeleccionado = 0; let validRisk = item.opciones.findIndex(opt => opt.cuota >= 2.2); 
      item.indexRiesgo = validRisk !== -1 ? validRisk : item.opciones.length - 1; encontroOpciones = true; 
    } 
  }); 
  window.dibujarTicketDinamico(false); 
  if (modo === 'props' && !encontroOpciones) { window.mostrarAlerta("Mercados Cerrados", "Casas de apuestas aún no habilitan Props.", "warning"); } 
};

window.rotarPickIndividual = function(id) { 
  let i = ticketDinamicoVIP.find(t => t.partido.id === id); 
  if(i) { 
    let opcionesValidas = []; 
    const isProp = (k) => k.includes('shots') || k.includes('corners') || k.includes('cards') || k.includes('player'); 
    if (modoMercadoGlobal === 'props') { i.opciones.forEach((o, idx) => { if(isProp(o.mercadoKey)) opcionesValidas.push(idx); }); } 
    else if (modoMercadoGlobal === 'mixto') { i.opciones.forEach((o, idx) => opcionesValidas.push(idx)); } 
    else { i.opciones.forEach((o, idx) => { if(o.mercadoKey === modoMercadoGlobal) opcionesValidas.push(idx); }); } 
    
    if(opcionesValidas.length > 0) { 
      let actualStr = modoRiesgoGlobal ? i.indexRiesgo : i.indexSeleccionado; 
      let arrayIdx = opcionesValidas.indexOf(actualStr); if(arrayIdx === -1) arrayIdx = 0; 
      let nextRealIdx = opcionesValidas[(arrayIdx + 1) % opcionesValidas.length]; 
      if(!modoRiesgoGlobal) { i.indexSeleccionado = nextRealIdx; } else { i.indexRiesgo = nextRealIdx; } 
    } 
    window.dibujarTicketDinamico(false); 
  } 
};

window.regenerarTicketCompleto = function() { 
  ticketDinamicoVIP.forEach(item => { 
    let max = item.opciones.length; 
    if(!modoRiesgoGlobal) { item.indexSeleccionado = Math.floor(Math.random() * (max > 4 ? 4 : max)); } 
    else { item.indexRiesgo = Math.floor(Math.random() * max); } 
  }); 
  window.dibujarTicketDinamico(false); 
};
...
window.dibujarTicketDinamico = function(esRadarAuto) {
  ...
  ticketDinamicoVIP.forEach((item, idx) => {
    const p = item.partido; const idxUsado = modoRiesgoGlobal ? item.indexRiesgo : item.indexSeleccionado; const o = item.opciones[idxUsado]; 
    cuotaTotal *= o.cuota; probPromedio += o.probabilidad;
    
    const defMercado = definicionesApuestas[o.mercadoKey] || { 'titulo': 'Mercado Especial' }; 
    let ico = "fa-handshake"; if(o.mercadoKey.includes('shots')) ico = "fa-bullseye"; else if(o.mercadoKey.includes('corners')) ico = "fa-flag"; else if(o.mercadoKey.includes('cards')) ico = "fa-square"; else if(o.mercadoKey === 'totals') ico = "fa-futbol"; else if(o.mercadoKey === 'spreads') ico = "fa-balance-scale";
    ...
    let badgeValor = ''; 
    if (o.verificado_ia) { badgeValor = `<span class="bg-blue-500/20 text-blue-400 border border-blue-500/50 px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-wider ml-2 shadow-sm animate-pulse"><i class="fas fa-gem"></i> Top Pick</span>`; } 
    ...
    <div class="${bg} p-4 rounded-xl mb-3 border border-white/5 relative overflow-hidden shadow-lg">
      ...
      <div class="flex justify-between items-center bg-black/60 p-3 rounded-lg border border-gray-700 shadow-inner">
        <div class="flex flex-col">
          ...
          <div class="mt-0.5 flex items-center"><span class="text-[8px] text-gray-500 uppercase font-bold"><i class="fas fa-shield-alt mr-1"></i> FR: </span>${badgeValor}</div>
        </div>
  ...
  <div class="card-glass p-4 border-t-2 ${modoRiesgoGlobal ? 'border-red-500 bg-red-500/5' : (esRadarAuto ? 'border-purple-500 bg-purple-500/5' : 'border-yellow-500 bg-yellow-500/5')} rounded-xl shadow-2xl mb-10">
    ...
    ${riskBanner} ${ctrls} ${htmlPartidos}
    <div class="flex justify-between items-end bg-black/60 p-4 rounded-xl border border-white/10 mt-4">
      <div class="flex flex-col"><span class="text-[10px] font-bold text-gray-400 uppercase">Cuota Final Calculada</span></div>
      <span class="text-3xl font-black ${modoRiesgoGlobal ? 'text-red-400' : 'text-white'}">${cuotaTotal.toFixed(2)}</span>
    </div>
    ${btnBar}
    <button onclick="window.guardarTicketHistorial('${cuotaTotal.toFixed(2)}')" class="w-full mt-2 py-4 bg-yellow-600 hover:bg-yellow-500 text-black rounded-xl text-[11px] font-black uppercase tracking-widest shadow-lg flex items-center justify-center gap-2 transition active:scale-95"><i class="fas fa-save text-lg"></i> GUARDAR EN HISTORIAL</button>
  </div>`;
};

// ==========================================
// 8. HISTORIAL DE TICKETS (USUARIO)
// ==========================================
...
window.limpiarHistorialSeguro = function() { window.mostrarConfirmacion("Limpiar Historial", "¿Borrar todos los tickets guardados?", () => { try { localStorage.removeItem('oracle_historial_' + codigoActivoUsuario); window.renderizarHistorial(); window.mostrarAlerta("Historial Limpio", "Tus tickets han sido eliminados.", "success"); } catch(e){} }); };
...
// ==========================================
// 10. APADRINAMIENTO Y FONDOS
// ==========================================
window.chequearApadrinamientoUI = function() {
  const bloqueado = document.getElementById('apadrinamientoBloqueado'); const onboard = document.getElementById('apadrinamientoOnboarding'); const dash = document.getElementById('apadrinamientoDashboard'); 
  if(!bloqueado || !onboard || !dash) return;
  if(!codigoActivoUsuario) { bloqueado.classList.remove('hidden'); onboard.classList.add('hidden'); dash.classList.add('hidden'); return; } 
...
window.iniciarApadrinamiento = async function() {
  ...
  } catch(e) { window.mostrarAlerta("Error de Servidor", "Google dice: " + e.message, "error"); } finally { if(btn) { btn.innerText = txtOriginal; btn.disabled = false; } }
};
...
window.generarRetoAdmin = async function() {
  ...
  try {
    ...
    // AQUÍ INYECTAMOS EL STAKE Y CONFIRMADOS POR DEFECTO PARA EL ADMIN
...
window.editarStakePickEscalera = async function(index, currentStake) {
  let n = prompt("Ingresa el % del fondo acumulado que se apostará en este pick (ej: 100, 50, 25):", currentStake); let val = parseFloat(n);
  if(!isNaN(val) && val > 0 && val <= 100) {
    try {
      ...
      if(snap.exists()) {
        ...
        window.cargarGestionRetoActivoAdmin(); window.mostrarAlerta("Éxito", "Porcentaje de inversión ajustado. Las proyecciones se recalcularon.", "success");
...
window.crearCodigo = async function(esIlimitado) {
  const inputCode = document.getElementById('newCodeInput'); if (!inputCode) return; const nuevoCodigo = inputCode.value.toUpperCase().trim();
  if (!nuevoCodigo || nuevoCodigo === 'UNDEFINED' || nuevoCodigo === 'NULL') { return window.mostrarAlerta("Error", "Debes ingresar un código alfanumérico válido.", "error"); }
  const ref = doc(db, "codigos_nube", nuevoCodigo);
  try {
    const docSnap = await getDoc(ref); if (docSnap.exists()) { return window.mostrarAlerta("Error", "Este código ya existe en la base de datos.", "error"); }
    const ahora = Date.now(); await setDoc(ref, { code: nuevoCodigo, ilimitado: esIlimitado, deviceID: null, ladderStatus: 'none', creado: ahora });
    inputCode.value = ''; window.mostrarAlerta("Éxito", `Código ${esIlimitado ? 'PREMIUM' : 'VIP'} creado.`, "success"); if(window.renderizarListaAdmin) window.renderizarListaAdmin();
...
window.renderizarListaAdmin = async function() {
  ...
  try { 
    const snap = await getDocs(collection(db, "codigos_nube")); l.innerHTML = ''; let arr = []; snap.forEach(d => arr.push(d.data())); arr.sort((a,b) => b.creado - a.creado); 
    arr.forEach(c => { 
      let col = c.ilimitado ? 'text-purple-400 bg-purple-900/20 border-purple-500/30' : 'text-yellow-500 bg-yellow-900/20 border-yellow-500/30'; let std = c.deviceID ? '<span class="text-red-400"><i class="fas fa-lock"></i> Usado</span>' : '<span class="text-green-400"><i class="fas fa-unlock"></i> Libre</span>'; let btnReset = c.deviceID ? `<button onclick="window.resetearDispositivo('${c.code}')" class="text-blue-400 hover:text-blue-300 transition-colors p-2 bg-blue-900/20 border border-blue-500/30 rounded-lg mr-2" title="Liberar Celular"><i class="fas fa-unlock-alt"></i></button>` : '';
...
window.cargarMonitorTickets = async function() {
  ...
  try { 
    ...
    snap.forEach(doc => { 
      ...
      lista.innerHTML += `<div class="bg-black/60 p-3 rounded-xl border border-yellow-500/20 relative shadow-md mb-3"><div class="absolute top-0 right-0 flex overflow-hidden rounded-bl-xl rounded-tr-xl shadow-md z-10"><button onclick="window.eliminarTicketAdmin('${doc.id}', 'dash')" class="bg-red-600 hover:bg-red-500 text-white text-[10px] px-3 py-1 transition-colors border-r border-red-700"><i class="fas fa-trash"></i></button><span class="bg-yellow-500 text-black text-[10px] font-black px-3 py-1">C: ${t.cuota}</span></div><div class="flex justify-between items-center mb-2 border-b border-white/5 pb-1 mt-1 pr-14"><span class="text-[11px] font-black text-white uppercase"><i class="fas fa-user-secret text-yellow-500 mr-1"></i> ${t.codigo_usuario}</span><span class="text-[8px] text-gray-500">${t.fecha}</span></div><div class="mt-2">${picksHtml}</div></div>`; 
    }); 
...
window.cargarUsuariosAdmin = async function(forzarRefresco = false) {
  ...
  try {
    ...
    snap.forEach(doc => { let t = doc.data(); t.id = doc.id; ticketsGlobalesAdminCache.push(t); let uid = t.codigo_usuario || 'Desconocido'; if(!mapaUsuarios[uid]) mapaUsuarios[uid] = { uid: uid, count: 0, last: t.timestamp }; mapaUsuarios[uid].count++; if(t.timestamp > mapaUsuarios[uid].last) mapaUsuarios[uid].last = t.timestamp; });
    window.usuariosAdminList = Object.values(mapaUsuarios).sort((a,b) => b.last - a.last); window.renderizarUsuariosFiltrados();
...
window.renderizarUsuariosFiltrados = function(filtro = '') {
  const contenedor = document.getElementById('listaUsuariosAdminContainer'); let filtrados = window.usuariosAdminList.filter(u => u.uid.toLowerCase().includes(filtro.toLowerCase()));
  ...
  let html = ''; filtrados.forEach(u => { html += `<button onclick="window.verDetalleUsuarioAdmin('${u.uid}')" class="w-full bg-black/40 hover:bg-black/60 border border-white/5 p-4 rounded-xl flex justify-between items-center transition-all shadow-sm active:scale-95 mb-2"><div class="flex items-center gap-3"><div class="bg-gray-800 p-3 rounded-full text-yellow-500 shadow-inner"><i class="fas fa-user-astronaut text-lg"></i></div><div class="flex flex-col items-start"><span class="text-white font-black text-xs uppercase tracking-wider">${u.uid}</span><span class="text-gray-400 text-[9px] font-bold mt-0.5 px-2 bg-gray-800 rounded">${u.count} Tickets Guardados</span></div></div><i class="fas fa-chevron-right text-gray-600"></i></button>`; }); contenedor.innerHTML = html;
};
...
window.verDetalleUsuarioAdmin = function(uid) {
  ...
  const contenedor = document.getElementById('ticketsUsuarioAdminContainer'); let ticketsUser = ticketsGlobalesAdminCache.filter(t => t.codigo_usuario === uid);
  ...
  ticketsUser.forEach(t => {
    ...
    if(arrayPicks.length > 0 && typeof arrayPicks[0] === 'object') {
      picksHtml = arrayPicks.map(p => { let mercado = p.mercadoKey || p.mercado || ''; const defMercado = definicionesApuestas[mercado] || { 'titulo': 'Mercado Especial' }; let ico = "fa-handshake"; if(mercado.includes('shots')) ico = "fa-bullseye"; else if(mercado.includes('corners')) ico = "fa-flag"; else if(mercado.includes('cards')) ico = "fa-square"; else if(mercado === 'totals') ico = "fa-futbol"; else if(mercado === 'spreads') ico = "fa-balance-scale"; let badgeIA = p.verificado_ia ? `<span class="text-blue-400 text-[8px] ml-1"><i class="fas fa-gem"></i></span>` : ''; let probBadge = p.probabilidad ? `<span class="bg-gray-800 text-gray-300 px-1 rounded text-[7px] ml-1">${p.probabilidad}%</span>` : ''; return `<div class="bg-gray-900/80 p-2 rounded mb-1 border border-white/5 relative"><div class="text-[7px] text-gray-400 font-bold uppercase mb-0.5"><i class="fas ${ico} mr-1"></i> ${defMercado.titulo}</div><div class="text-[9px] font-bold text-white leading-tight mb-1">${p.partido}</div><div class="flex justify-between items-center"><span class="text-[8px] text-yellow-500 font-black uppercase">PICK: ${p.pick} ${badgeIA} ${probBadge}</span><span class="text-white font-black text-[10px]">${p.cuota ? parseFloat(p.cuota).toFixed(2) : ''}</span></div></div>`; }).join('');
    } else if (arrayPicks.length > 0) { picksHtml = arrayPicks.map(p => `<li class="text-[8px] text-gray-300 border-b border-white/5 py-1 last:border-0">${p}</li>`).join(''); }
    html += `<div class="bg-black/60 p-3 rounded-xl border border-white/10 relative shadow-md mb-3"><div class="absolute top-0 right-0 flex overflow-hidden rounded-bl-xl rounded-tr-xl shadow-md z-10"><button onclick="window.eliminarTicketAdmin('${t.id}', 'users')" class="bg-red-600 hover:bg-red-500 text-white text-[10px] px-3 py-1 transition-colors border-r border-red-700"><i class="fas fa-trash"></i></button><span class="bg-yellow-500 text-black text-[10px] font-black px-3 py-1">C: ${t.cuota}</span></div><div class="flex justify-between items-center mb-2 border-b border-white/5 pb-1 mt-1 pr-14"><span class="text-[10px] font-bold text-gray-400"><i class="far fa-calendar-alt mr-1"></i> ${t.fecha}</span></div><div class="mt-2">${picksHtml}</div></div>`;
  }); contenedor.innerHTML = html;
};
...
window.cargarFondosAdmin = async function() {
  ...
  try {
    const q = query(collection(db, "codigos_nube")); const snap = await getDocs(q); let html = '';
    snap.forEach(doc => {
      let data = doc.data();
      if (data.apadrinamiento && data.apadrinamiento.activo) {
        let inicial = data.apadrinamiento.bankroll_inicial; let actual = data.apadrinamiento.bankroll_actual; let rendimiento = (((actual - inicial) / inicial) * 100).toFixed(2); let colorRendimiento = rendimiento > 0 ? 'text-green-500' : (rendimiento < 0 ? 'text-red-500' : 'text-gray-400');
        html += `<button onclick="window.verDetalleFondoAdmin('${data.code}')" class="w-full bg-black/40 hover:bg-black/60 border border-white/5 p-4 rounded-xl flex justify-between items-center transition-all shadow-sm active:scale-95 mb-2"><div class="flex items-center gap-3"><div class="bg-gray-800 p-3 rounded-full text-green-500 shadow-inner"><i class="fas fa-piggy-bank text-lg"></i></div><div class="flex flex-col items-start"><span class="text-white font-black text-xs uppercase tracking-wider">${data.code}</span><span class="text-gray-400 text-[9px] font-bold mt-0.5 px-2 bg-gray-800 rounded">Banca: ${formatoCOP(actual)}</span></div></div><div class="flex flex-col items-end"><span class="${colorRendimiento} font-black text-sm">${rendimiento}%</span></div></button>`;
      }
...
window.verDetalleFondoAdmin = async function(codigo) {
  document.getElementById('panelListaFondoAdmin').classList.add('hidden'); document.getElementById('panelDetalleFondoAdmin').classList.remove('hidden'); document.getElementById('tituloDetalleFondoAdmin').innerText = `PORTAFOLIO DE: ${codigo}`;
  const statsContainer = document.getElementById('statsFondoAdmin'); const ticketsContainer = document.getElementById('ticketsFondoAdminContainer');
  statsContainer.innerHTML = '<div class="col-span-2 text-center text-xs text-green-500"><i class="fas fa-spinner fa-spin"></i> Cargando...</div>'; ticketsContainer.innerHTML = '';
  try {
    const docSnap = await getDoc(doc(db, "codigos_nube", codigo));
    if (docSnap.exists()) {
      let data = docSnap.data().apadrinamiento; let rendimiento = (((data.bankroll_actual - data.bankroll_inicial) / data.bankroll_inicial) * 100).toFixed(2); let color = rendimiento > 0 ? 'text-green-400' : (rendimiento < 0 ? 'text-red-400' : 'text-gray-400');
      statsContainer.innerHTML = `<div class="bg-gray-900/80 p-3 rounded-lg border border-white/5 flex flex-col items-center shadow-inner"><span class="text-[8px] text-gray-500 uppercase font-bold">Capital Inicial</span><span class="text-white font-black text-xs">${formatoCOP(data.bankroll_inicial)}</span></div><div class="bg-black/40 p-3 rounded-lg border border-green-500/30 flex flex-col items-center shadow-[0_0_10px_rgba(34,197,94,0.1)]"><span class="text-[8px] text-green-500 uppercase font-bold">Bankroll Actual</span><span class="${color} font-black text-sm">${formatoCOP(data.bankroll_actual)}</span><span class="${color} text-[8px] font-bold">ROI: ${rendimiento}%</span></div>`;
    }
    const q = query(collection(db, "tickets_apadrinamiento"), where("codigo_usuario", "==", codigo)); const snapTickets = await getDocs(q);
    ...
    let tArr = []; snapTickets.forEach(d => { let tk = d.data(); tk.id = d.id; tArr.push(tk); }); tArr.sort((a,b) => b.timestamp - a.timestamp);
    let htmlTickets = '';
    tArr.forEach(t => {
      let bgStatus = 'bg-gray-800/50 border-gray-600'; let badgeStatus = '<span class="bg-gray-700 text-gray-300 px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest shadow-sm"><i class="far fa-clock"></i> Pendiente</span>'; let controlButtons = '';
      if (t.estado === 'pendiente') { controlButtons = `<div class="flex gap-2 mt-3 pt-3 border-t border-white/5"><button onclick="window.resolverTicketFondo('${t.id}', '${codigo}', 'won', ${t.monto_apostar}, ${t.cuota_usuario})" class="flex-1 py-2 bg-green-600/20 hover:bg-green-600/40 text-green-500 border border-green-500/30 rounded-lg text-[9px] font-black uppercase transition-colors"><i class="fas fa-check mr-1"></i> Pagar (+)</button><button onclick="window.resolverTicketFondo('${t.id}', '${codigo}', 'lost', ${t.monto_apostar}, ${t.cuota_usuario})" class="flex-1 py-2 bg-red-600/20 hover:bg-red-600/40 text-red-500 border border-red-500/30 rounded-lg text-[9px] font-black uppercase transition-colors"><i class="fas fa-times mr-1"></i> Restar (-)</button></div>`; } else if (t.estado === 'won') { bgStatus = 'bg-green-900/20 border-green-500/50 shadow-[0_0_15px_rgba(34,197,94,0.1)]'; badgeStatus = '<span class="bg-green-600 text-white px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest shadow-md"><i class="fas fa-check-circle"></i> Ganada</span>'; } else if (t.estado === 'lost') { bgStatus = 'bg-red-900/20 border-red-500/50 shadow-[0_0_15px_rgba(239,68,68,0.1)]'; badgeStatus = '<span class="bg-red-600 text-white px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest shadow-md"><i class="fas fa-times-circle"></i> Perdida</span>'; }
      let picksHtml = t.picks.map(p => { return `<div class="bg-gray-900/60 p-2 rounded mb-1 border border-white/5"><div class="text-[9px] font-bold text-white truncate">${p.partido}</div><div class="flex justify-between items-center mt-1"><span class="text-[8px] text-yellow-500 font-black uppercase">PICK: ${p.pick}</span></div></div>`; }).join('');
      htmlTickets += `<div class="${bgStatus} p-3 rounded-xl border relative mb-3 transition-all"><div class="flex justify-between items-center mb-2 border-b border-white/10 pb-2"><span class="text-[9px] font-bold text-gray-400"><i class="far fa-calendar-alt"></i> ${t.fecha}</span>${badgeStatus}</div><div class="mb-2">${picksHtml}</div><div class="flex justify-between items-end bg-black/60 p-2 rounded-lg border border-black shadow-inner"><div class="flex flex-col"><span class="text-[7px] text-gray-500 uppercase font-bold">Inversión (${t.stake_porcentaje}%)</span><span class="text-xs font-black text-white">${formatoCOP(t.monto_apostar)}</span></div><div class="flex flex-col text-right"><span class="text-[7px] text-gray-500 uppercase font-bold">Cuota del Cliente</span><span class="text-xs font-black text-yellow-500">${t.cuota_usuario.toFixed(2)}</span></div></div>${controlButtons}</div>`;
    }); ticketsContainer.innerHTML = htmlTickets;
  } catch (e) { console.error(e); ticketsContainer.innerHTML = '<p class="text-red-500 text-xs text-center">Error al cargar datos del fondo.</p>'; }
};
...
window.resolverTicketFondo = async function(ticketId, codigoUsuario, estado, montoApostar, cuota) {
  window.mostrarConfirmacion("Matemática de Fondo", `¿Confirmas que esta operación fue ${estado === 'won' ? 'GANADA' : 'PERDIDA'}? Se ajustará automáticamente el saldo del cliente.`, async () => {
    try {
      const ticketRef = doc(db, "tickets_apadrinamiento", ticketId); const userRef = doc(db, "codigos_nube", codigoUsuario);
      const uSnap = await getDoc(userRef); if(!uSnap.exists()) return;
      let bankActual = uSnap.data().apadrinamiento.bankroll_actual;
      if (estado === 'won') { let gananciaNeta = (montoApostar * cuota) - montoApostar; bankActual += gananciaNeta; } else if (estado === 'lost') { bankActual -= montoApostar; }
      await updateDoc(ticketRef, { estado: estado }); await updateDoc(userRef, { "apadrinamiento.bankroll_actual": bankActual });
      window.mostrarAlerta("Operación Liquidada", "Saldo del cliente actualizado exitosamente.", "success"); window.verDetalleFondoAdmin(codigoUsuario); 
    } catch (e) { window.mostrarAlerta("Error", "No se pudo actualizar el balance.", "error"); }
  });
};
