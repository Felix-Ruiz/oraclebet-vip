...
window.cerrarModalLogin = function() { 
    ...
    if(btn) { 
        btn.innerHTML = 'VERIFICAR ACCESO'; 
    } 
};
...
// 🚀 FIX 1: LA BASE DE DATOS SE CARGA PRIMERO PARA TODOS LOS USUARIOS
...
// 🚀 FIX 2: ELIMINACIÓN DE ESPACIOS INVISIBLES AL METER EL CORREO
    ...
    if (codigoIngresado.startsWith("MASTER_")) {
        try {
            if (!correoAdminTemp) {
                ...
                correoAdminTemp = pmt.trim(); // 🚀 DESTRUYE ESPACIOS INVISIBLES AL FINAL
            }
...
window.enviarNotificacionGlobal = async function() {
    ...
    if(!titulo || !cuerpo) { 
        window.mostrarAlerta("Campos Vacíos", "Ingresa un título y un mensaje.", "warning"); 
        return; 
    }
    ...
    try {
        ...
        btn.innerHTML = `<i class="fas fa-paper-plane mr-1"></i> Notificar a Inversores`; 
        btn.disabled = false;
    }
}

window.cargarNotificacionesAdmin = async function() {
    const lista = document.getElementById('adminNotificacionesList'); 
    if(!lista) return;
    ...
    try {
        ...
        if(snap.empty) { 
            lista.innerHTML = `<p class="text-[10px] text-gray-500 text-center border border-dashed border-white/10 p-4 rounded-lg">No hay comunicados enviados.</p>`; 
            return; 
        }
        
        snap.forEach(doc => {
            let data = doc.data();
            let f = new Date(data.timestamp).toLocaleDateString('es-CO', {
                month: 'short', 
                day: '2-digit', 
                hour: '2-digit', 
                minute: '2-digit'
            ...
            let audBadge = data.audiencia === "escalera" 
                ? '<span class="bg-yellow-500/20 text-yellow-500 px-1 rounded ml-1">Escalera</span>' 
                : '<span class="bg-blue-500/20 text-blue-400 px-1 rounded ml-1">Todos</span>';
        ...
        lista.innerHTML = '<p class="text-red-500 text-xs text-center">Error al cargar.</p>'; 
    }
};
...
window.eliminarNotificacionAdmin = async function(idDoc) {
    window.mostrarConfirmacion("Eliminar Comunicado", "¿Deseas borrar este mensaje? Desaparecerá de la bandeja de todos los usuarios de forma inmediata.", async () => {
        try {
            ...
            window.mostrarAlerta("Error", "No se pudo borrar la notificación.", "error"); 
        }
    });
};
...
window.renderizarLayoutAdmin = function() {
    ...
    const fApp = document.getElementById('footerApp'); 
    if(bNav) bNav.style.display = 'none'; 
    if(fApp) fApp.style.display = 'none'; 
        ...
        <div class="p-4 bg-gray-900 min-h-screen pb-20">
            ...
            <div id="vistaAdm_ladder" class="admin-view-content hidden space-y-4">
                ...
                <div class="bg-black/60 p-5 rounded-2xl border border-yellow-500/20 shadow-lg relative overflow-hidden">
                    ...
                    <div class="space-y-4">
                        <div>
                            <label class="text-[9px] text-gray-400 font-bold uppercase mb-1.5 block ml-1"><i class="fas fa-dollar-sign mr-1"></i> Capital Inicial (Fondo)</label>
                            <input type="number" id="inputCapitalEscalera" value="50000" class="w-full bg-gray-900 border border-white/10 p-3.5 rounded-xl text-white font-black text-sm outline-none focus:border-yellow-500 shadow-inner">
                        </div>
                        <div>
                            <label class="text-[9px] text-gray-400 font-bold uppercase mb-1.5 block ml-1"><i class="fas fa-crosshairs mr-1"></i> Cuota Objetivo Global</label>
                            <input type="number" id="inputCuotaObjetivo" step="0.1" value="2.0" class="w-full bg-gray-900 border border-white/10 p-3.5 rounded-xl text-white font-black text-sm outline-none focus:border-yellow-500 shadow-inner">
                        </div>
                        <div>
                            <label class="text-[9px] text-gray-400 font-bold uppercase mb-1.5 block ml-1"><i class="fas fa-shield-alt mr-1"></i> Seguridad Mínima (%)</label>
                            <input type="number" id="inputProbMinima" value="85" class="w-full bg-gray-900 border border-white/10 p-3.5 rounded-xl text-white font-black text-sm outline-none focus:border-yellow-500 shadow-inner">
                        </div>
                        <div>
                            ...
                            <input type="date" id="inputFechaEscalera" class="w-full bg-gray-900 border border-white/10 p-3.5 rounded-xl text-white font-black text-xs uppercase outline-none focus:border-yellow-500 shadow-inner">
                        </div>
            ...
            m.outcomes?.forEach(o => { 
                ...
                if(o.price > 1.0) { 
                    ...
                    mapaResultados[k].brokers.push({ 
                        broker: b.title, 
                        cuota: o.price, 
                        probabilidad_real: o.probabilidad_real || null, 
                        ev_porcentaje: o.ev_porcentaje || null, 
                        es_valor: o.es_valor || false, 
                        verificado_ia: o.verificado_ia || false 
                    }); 
                } 
            }); 
        }); 
        ...
        if(res.cuotas.length >= liquidezMinima) { 
            ...
            if(mejorOpcion.cuota >= 1.05 && mejorOpcion.cuota <= 4.50) { 
                opcionesFinales.push({ 
                    broker: mejorOpcion.broker, 
                    mercadoKey: res.mercadoKey, 
                    nombre: res.nombre, 
                    point: res.point, 
                    cuota: mejorOpcion.cuota, 
                    probabilidad: probFinal, 
                    edgeValor: edgeFinal, 
                    es_valor: mejorOpcion.es_valor || (edgeFinal > 0), 
                    verificado_ia: mejorOpcion.verificado_ia 
                }); 
            } 
        } 
    });
...
window.mejorarProbabilidadTicket = function() {
            ...
            b.markets?.forEach(m => { 
                m.outcomes?.forEach(o => { 
                    ...
                    if (oOriginal.mercadoKey === 'h2h' && m.key === 'spreads' && !origLower.includes('draw') && !origLower.includes('empate') && esMismoEquipo) { 
                        ...
                        if ((nombreLower.includes('over') || origLower.includes('over') || nombreLower.includes('más') || origLower.includes('más')) && pointNum < parseFloat(oOriginal.point)) { 
                            esCandidato = true; 
                        } 
                    } else if (oOriginal.mercadoKey.includes('totals') && m.key === oOriginal.mercadoKey) { 
                        if ((nombreLower.includes('under') || origLower.includes('under') || nombreLower.includes('menos') || origLower.includes('menos')) && pointNum > parseFloat(oOriginal.point)) { 
                            esCandidato = true; 
                        } 
                    ...
                    if (esCandidato && o.price >= 1.05 && o.price < oOriginal.cuota) { 
                        ...
                        candidatos.push({ 
                            ...
                            point: o.point, 
                            cuota: o.price, 
                            probabilidad: probAprox, 
                            edgeValor: edgeMejora, 
                            es_valor: edgeMejora > 0, 
                            verificado_ia: oOriginal.verificado_ia, 
                            es_mejora: true 
                        }); 
                    } 
                }); 
    ...
    if (huboMejoras) { 
        window.mostrarAlerta("Ticket Blindado", "Se han encontrado líneas de protección bajando drásticamente el riesgo.", "success"); 
        window.dibujarTicketDinamico(false); 
    } else { 
        window.mostrarAlerta("Límites Alcanzados", "El mercado no ofrece Hándicaps Asiáticos o líneas más seguras para estos equipos en este momento.", "warning"); 
    }
};

window.toggleModoRiesgo = function() { 
    modoRiesgoGlobal = !modoRiesgoGlobal; 
    window.dibujarTicketDinamico(false); 
};
...
window.cambiarModoMercado = function(modo) { 
    ...
    ticketDinamicoVIP.forEach(item => { 
        ...
        if(modo === 'props') { 
            ...
            if (idxS !== -1) { 
                item.indexSeleccionado = idxS; 
                let lastIdxR = -1; 
                item.opciones.forEach((o, i) => { if (isProp(o.mercadoKey)) lastIdxR = i; }); 
                item.indexRiesgo = lastIdxR !== -1 ? lastIdxR : idxS; 
            ...
            let validRisk = item.opciones.findIndex(opt => opt.cuota >= 2.2); 
            item.indexRiesgo = validRisk !== -1 ? validRisk : item.opciones.length - 1; 
            encontroOpciones = true; 
        } 
    }); 
    ...
    if (modo === 'props' && !encontroOpciones) { 
        window.mostrarAlerta("Mercados Cerrados", "Casas de apuestas aún no habilitan Props.", "warning"); 
    } 
};
...
window.rotarPickIndividual = function(id) { 
    ...
    if(i) { 
        ...
        if (modoMercadoGlobal === 'props') { 
            i.opciones.forEach((o, idx) => { if(isProp(o.mercadoKey)) opcionesValidas.push(idx); }); 
        } else if (modoMercadoGlobal === 'mixto') { 
            i.opciones.forEach((o, idx) => opcionesValidas.push(idx)); 
        } else { 
            i.opciones.forEach((o, idx) => { if(o.mercadoKey === modoMercadoGlobal) opcionesValidas.push(idx); }); 
        } 
        
        if(opcionesValidas.length > 0) { 
            let actualStr = modoRiesgoGlobal ? i.indexRiesgo : i.indexSeleccionado; 
            let arrayIdx = opcionesValidas.indexOf(actualStr); 
            if(arrayIdx === -1) arrayIdx = 0; 
            let nextRealIdx = opcionesValidas[(arrayIdx + 1) % opcionesValidas.length]; 
            
            if(!modoRiesgoGlobal) { 
                i.indexSeleccionado = nextRealIdx; 
            } else { 
                i.indexRiesgo = nextRealIdx; 
            } 
        } 
        window.dibujarTicketDinamico(false); 
    } 
...
window.regenerarTicketCompleto = function() { 
    ticketDinamicoVIP.forEach(item => { 
        ...
        if(!modoRiesgoGlobal) { 
            item.indexSeleccionado = Math.floor(Math.random() * (max > 4 ? 4 : max)); 
        } else { 
            item.indexRiesgo = Math.floor(Math.random() * max); 
        } 
    }); 
    window.dibujarTicketDinamico(false); 
};
        ...
        if(snap.exists()) { 
            ...
            if(tk && tk.picks) { 
                tk.picks.forEach((p, index) => { 
                    ...
                    let badgeProgreso = '';
                    if (estadoP === 'won') { 
                        badgeProgreso = `<div class="bg-green-900/40 border border-green-500/50 text-green-400 text-[10px] font-black p-2 rounded flex justify-between items-center w-full shadow-inner mt-2"><span class="text-[8px] text-gray-400 uppercase">Fondo Acumulado</span><span>${startPctGlobal.toFixed(1)}% <i class="fas fa-arrow-right text-[8px] text-green-500 mx-1"></i> ${endPctGlobal.toFixed(1)}%</span></div>`; 
                    } else if (estadoP === 'lost') { 
...
window.chequearApadrinamientoUI = function() {
    ...
    if(!codigoActivoUsuario) { 
        bloqueado.classList.remove('hidden'); onboard.classList.add('hidden'); dash.classList.add('hidden'); 
        return; 
    } 
    bloqueado.classList.add('hidden');
    
    if (!perfilApadrinamiento || !perfilApadrinamiento.activo) { 
        onboard.classList.remove('hidden'); dash.classList.add('hidden'); 
    } else { 
        onboard.classList.add('hidden'); dash.classList.remove('hidden'); window.renderizarDashboardApadrinamiento(); 
    }
};
    ...
    try { 
        unsubscribeApadrinamiento = onSnapshot(doc(db, "codigos_nube", codigoActivoUsuario), (docSnap) => { 
            if(docSnap.exists()) { 
                ...
                if(document.getElementById('vista_escalera')?.classList.contains('view-active')) { window.chequearEstadoEscaleraUI(); } 
                if(document.getElementById('vista_apadrinamiento')?.classList.contains('view-active')) { window.chequearApadrinamientoUI(); } 
            } 
        }); 
    } catch(e) {} 
};
...
window.iniciarApadrinamiento = async function() {
    ...
    try {
        await setDoc(doc(db, "codigos_nube", usuarioActual), { 
            apadrinamiento: { bankroll_inicial: monto, bankroll_actual: monto, fecha_inicio: Date.now(), activo: true } 
        }, { merge: true });
        ...
        if(dash) { dash.style.display = 'block'; dash.classList.remove('hidden'); }
        if(window.cargarDatosApadrinamiento) window.cargarDatosApadrinamiento();
    } catch(e) { 
        console.error("Fallo en activación:", e); window.mostrarAlerta("Error de Servidor", "Google dice: " + e.message, "error"); 
    } finally { 
...
window.renderizarDashboardApadrinamiento = async function() {
    ...
    if (rendimiento > 0) { 
        uiRendimiento.innerText = `Rendimiento: +${rendimiento.toFixed(2)}%`; uiRendimiento.className = "text-[10px] font-bold text-green-400 mt-1"; uiBadge.className = "absolute top-0 right-0 bg-green-600 text-white text-[8px] font-black px-2 py-1 rounded-bl-lg"; 
    } else if (rendimiento < 0) { 
        uiRendimiento.innerText = `Rendimiento: ${rendimiento.toFixed(2)}%`; uiRendimiento.className = "text-[10px] font-bold text-red-400 mt-1"; uiBadge.className = "absolute top-0 right-0 bg-red-600 text-white text-[8px] font-black px-2 py-1 rounded-bl-lg"; 
    } else { 
        uiRendimiento.innerText = `Rendimiento: 0.00%`; uiRendimiento.className = "text-[10px] font-bold text-gray-400 mt-1"; uiBadge.className = "absolute top-0 right-0 bg-gray-600 text-white text-[8px] font-black px-2 py-1 rounded-bl-lg"; 
    }
    ...
    if (perfilApadrinamiento.ultimo_dia_generado === hoy) { 
        btn.innerHTML = `<i class="fas fa-check-circle text-lg"></i> OPERACIÓN LISTA`; btn.className = "w-full py-4 bg-gray-800 text-gray-500 rounded-xl font-black text-[12px] uppercase cursor-not-allowed border border-gray-700"; btn.disabled = true; 
    } else { 
        btn.innerHTML = `<i class="fas fa-search-dollar text-lg"></i> BUSCAR OPORTUNIDAD`; btn.className = "w-full py-4 bg-gradient-to-r from-yellow-600 to-yellow-500 text-black rounded-xl font-black text-[12px] uppercase shadow-[0_10px_20px_rgba(212,175,55,0.3)] transition-all active:scale-95"; btn.disabled = false; 
    } 
...
window.generarTicketApadrinamiento = async function() {
    if(!perfilApadrinamiento) return; 
    const btn = document.getElementById('btnGenerarApadrinamiento'); 
    btn.innerHTML = `<i class="fas fa-spinner fa-spin text-lg"></i> ESCANEANDO MERCADO...`; btn.disabled = true;
    let previewExistente = document.getElementById('previewArriesgar'); 
    if(previewExistente) previewExistente.remove();
    ...
    setTimeout(async () => {
        ...
        if (pickUnicoD) { 
            ticketFinal.push(pickUnicoD); cuotaAlcanzada = pickUnicoD.opcion.cuota; probPromedio = pickUnicoD.opcion.probabilidad; 
        } else { 
                ...
                for(let j = i + 1; j < opcionesDiamante.length && !encontrado; j++) { 
                    if (opcionesDiamante[i].partido.id !== opcionesDiamante[j].partido.id) { 
                        ...
                        if (combinada >= 1.30 && combinada <= 1.60) { 
                            ticketFinal.push(opcionesDiamante[i]); ticketFinal.push(opcionesDiamante[j]); cuotaAlcanzada = combinada; probPromedio = (opcionesDiamante[i].opcion.probabilidad + opcionesDiamante[j].opcion.probabilidad) / 2; encontrado = true; 
                        } 
                    } 
                } 
            } 
        ...
        if (ticketFinal.length === 0) { 
            esPlanB = true; let pickUnicoR = opcionesRespaldo.find(x => x.opcion.cuota >= 1.30 && x.opcion.cuota <= 1.55); 
            if (pickUnicoR) { 
                ticketFinal.push(pickUnicoR); cuotaAlcanzada = pickUnicoR.opcion.cuota; probPromedio = pickUnicoR.opcion.probabilidad; 
            } else { 
                ...
                for(let i = 0; i < opcionesRespaldo.length && !encontradoR; i++) { 
                    for(let j = i + 1; j < opcionesRespaldo.length && !encontradoR; j++) { 
                        if (opcionesRespaldo[i].partido.id !== opcionesRespaldo[j].partido.id) { 
                            ...
                            if (combinada >= 1.30 && combinada <= 1.60) { 
                                ticketFinal.push(opcionesRespaldo[i]); ticketFinal.push(opcionesRespaldo[j]); cuotaAlcanzada = combinada; probPromedio = (opcionesRespaldo[i].opcion.probabilidad + opcionesRespaldo[j].opcion.probabilidad) / 2; encontradoR = true; 
                            } 
                        } 
                    } 
                } 
        ...
        let objTicket = { 
            id: ticketId, codigo_usuario: codigoActivoUsuario, fecha: hoyStr, cuota_sistema: parseFloat(cuotaAlcanzada.toFixed(2)), cuota_usuario: parseFloat(cuotaAlcanzada.toFixed(2)), stake_porcentaje: porcentajeStake, monto_apostar: montoApostar, estado: 'pendiente', timestamp: Date.now(), picks: ticketFinal.map(t => {
        ...
        if (esPlanB) {
            ...
            try { 
                ...
                window.mostrarAlerta("Error", "No se pudo guardar.", "error"); window.renderizarDashboardApadrinamiento(); 
            }
        }
    }, 1500); 
};
...
window.generarRetoAdmin = async function() {
    ...
    try {
        ...
        window.retoPendientePublicar = { 
            picks: seleccionados.map(s => ({ home_team: s.partido.home_team, away_team: s.partido.away_team, mercadoKey: s.mercadoKey, nombre: s.nombre, point: s.point === undefined ? null : s.point, cuota: s.cuota, probabilidad: s.probabilidad, stake: 100, confirmados: [] })), 
            cuotaTotal: cuotaAcum.toFixed(2), fechaFiltro: fechaElegida 
        };
...
window.renderizarListaAdmin = async function() {
    ...
    try { 
        ...
        a.forEach(c => { 
            ...
            let bR = c.deviceID ? `<button onclick="window.resetearDispositivo('${c.code}')" class="text-blue-400 hover:text-blue-300 transition-colors p-2 bg-blue-900/20 border border-blue-500/30 rounded-lg mr-2"><i class="fas fa-unlock-alt"></i></button>` : ''; 
